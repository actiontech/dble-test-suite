...
select a.id,a.t_id,b.o_id,b.name from (select * from test1 where id<3) a,(select * from schema2.test2 where id>3) b
select a.id,b.id,b.pad,a.t_id from test1 a,(select schema3.test3.id,schema3.test3.pad from test1 join schema3.test3 where test1.pad=schema3.test3.pad) b,(select * from schema2.test2 where id>3) c where a.pad=b.pad and c.pad=b.pad
#
#join table
#
select a.id,a.t_id,a.name,a.pad,b.id,b.o_id,b.name,b.pad from test1 a join schema2.test2 as b order by a.id,b.id
select a.id,a.t_id,a.name,a.pad,b.id,b.o_id,b.name,b.pad from test1 a inner join schema2.test2 b order by a.id,b.id
select a.id,a.t_id,a.name,a.pad,b.id,b.o_id,b.name,b.pad from test1 a cross join schema2.test2 b order by a.id,b.id
select a.id,a.name,a.pad,b.name from test1 a straight_join schema2.test2 b on a.pad=b.pad
#
#SELECT ... UNION [ALL | DISTINCT] SELECT ... [UNION [ALL | DISTINCT] SELECT ...]
#
select a.id,a.t_id,a.name,a.pad from test1 a union all select b.id,b.m_id,b.name,b.pad from schema3.test3 b union all select c.id,c.o_id,c.name,c.pad from schema2.test2 c
select a.id,a.t_id,a.name,a.pad from test1 a union distinct select b.id,b.m_id,b.name,b.pad from schema3.test3 b union distinct select c.id,c.o_id,c.name,c.pad from schema2.test2 c
(select name from test1 where pad=1 order by id limit 10) union all (select name from schema2.test2 where pad=1 order by id limit 10)/*allow_diff_sequence*/
(select name from test1 where pad=1 order by id limit 10) union distinct (select name from schema2.test2 where pad=1 order by id limit 10)/*allow_diff_sequence*/
