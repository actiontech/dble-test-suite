...
select a.id,b.id,b.pad,a.t_id from sharding_4_t1 a,(select all * from noshard_t1) b where a.t_id=b.o_id;
select a.id,b.id,b.pad,a.t_id from sharding_4_t1 a,(select distinct * from noshard_t1) b where a.t_id=b.o_id;
select * from (select * from noshard_t1 a group by a.id) a;
select * from (select pad,count(*) from noshard_t1 a group by pad) a;
select a.id,b.id,b.pad,a.t_id from sharding_4_t1 a,(select * from noshard_t1 having pad>3) b where a.t_id=b.o_id;
select a.id,b.id,b.pad,a.t_id from sharding_4_t1 a,(select * from noshard_t1 where pad>3 order by id) b where a.t_id=b.o_id;
select a.id,b.id,b.pad,a.t_id from sharding_4_t1 a,(select * from noshard_t1 order by id limit 3) b where a.t_id=b.o_id;
select a.id,b.id,b.pad,a.t_id from sharding_4_t1 a,(select * from noshard_t1 order by id limit 3) b where a.t_id=b.o_id limit 2;
select a.id,b.id,b.pad,a.t_id from sharding_4_t1 a,(select * from noshard_t1 where pad>3) b where a.t_id=b.o_id;
select * from (select noshard_t1.pad from sharding_4_t1 left join noshard_t1 on sharding_4_t1.pad=noshard_t1.pad) a;
select * from (select * from sharding_4_t1 union select * from noshard_t1) a where a.id >3;
select id,pad from sharding_4_t1 where pad=(select min(id) from noshard_t1);
...
select * from sharding_4_t1 where pad<(select pad from noshard_t1 where id=3);
select * from sharding_4_t1 having pad<(select pad from noshard_t1 where id=3);
select a.id,b.id,b.pad,a.t_id from sharding_4_t1 a,(select * from noshard_t1 where pad>3) b where a.t_id=b.o_id;
select id,name,(select count(*) from noshard_t1) count from sharding_4_t1;
select * from sharding_4_t1 where pad like (select pad from noshard_t1 where id=3);
...
select * from sharding_4_t1,(select * from sharding_4_t1 where id>3 union select * from noshard_t1 where id<2) a where a.id >3 and sharding_4_t1.pad=a.pad;
select count(*) from (select * from sharding_4_t1 where pad=(select pad from noshard_t1 where id=1)) a;
#
#Second supplement
#
select (select name from sharding_4_t1 order by name limit 1)
select * from sharding_4_t1 where 'test_2'=(select name from noshard_t1 where id=2)
select * from sharding_4_t1 where 5=(select count(*) from noshard_t1)
select * from sharding_4_t1 where 'test_2' like(select name from noshard_t1 where id=2)
...
select * from sharding_4_t1 where (id,pad)=(select id,pad from noshard_t1 limit 1)
select * from sharding_4_t1 where row(id,pad)=(select id,pad from noshard_t1 limit 1)
select id,name,pad from sharding_4_t1 where (id,pad)in(select id,pad from noshard_t1)
select id,name,pad from sharding_4_t1 where (1,1)in(select id,pad from noshard_t1)
SELECT pad FROM sharding_4_t1 AS x WHERE x.id = (SELECT pad FROM noshard_t1 AS y WHERE x.id = (SELECT pad FROM schema2.sharding_4_t2 WHERE y.id = schema2.sharding_4_t2.id))
