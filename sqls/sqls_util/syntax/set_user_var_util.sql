...
create table test1(id int(4), B int(4))
insert into test1 values (10, 1),(10, 1),(11,1),(10, 1)
create table schema2.test2(id int(4), B int(4))
insert into schema2.test2 values (10, 1),(11, 2),(10,2)
create table schema3.test3(id int(4), B int(4))
insert into schema3.test3 values (12, 1),(11, 1),(10,11)
(SELECT @vara:=id FROM test1 WHERE id=10 AND B=1) UNION (SELECT @varb:=B FROM schema2.test2 WHERE id=@vara AND B=2) UNION (SELECT id FROM schema3.test3 WHERE id=@vara+@varb AND B=1)
...
DROP TABLE IF EXISTS test1
create table test1(id int(4), dealer varchar(10), price float(8,2))
...
SELECT (@aa:=id) AS a, (@aa+3) AS b FROM test1 HAVING b=5
