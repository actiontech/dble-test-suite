...
select * from aly_test use key for order by(k_1)
select * from aly_test ignore key for order by(k_1)
select * from aly_test force key for order by(k_1)
...
select * from aly_test a,a_order_no_shard b
select * from (select * from aly_test where id<3) a,(select * from a_order_no_shard where id>3) b
select a.id,b.id,b.pad,a.t_id from aly_test a,(select a_manager.id,a_manager.pad from aly_test join a_manager where aly_test.pad=a_manager.pad) b,(select * from a_order_no_shard where id>3) c where a.pad=b.pad and c.pad=b.pad
#
#join table
#
select * from aly_test a join a_order_no_shard as b order by a.id,b.id
select * from aly_test a inner join a_order_no_shard b order by a.id,b.id
select * from aly_test a cross join a_order_no_shard b order by a.id,b.id
select a.id,a.name,a.pad,b.name from aly_test a straight_join a_order_no_shard b on a.pad=b.pad
#
#SELECT ... UNION [ALL | DISTINCT] SELECT ... [UNION [ALL | DISTINCT] SELECT ...]
#
select * from aly_test union all select * from a_manager union all select * from a_order_no_shard
select * from aly_test union distinct select * from a_manager union distinct select * from a_order_no_shard
(select name from aly_test where pad=1 order by id limit 10) union all (select name from a_order_no_shard where pad=1 order by id limit 10) order by name
(select name from aly_test where pad=1 order by id limit 10) union distinct (select name from a_order_no_shard where pad=1 order by id limit 10) order by name
(select * from aly_test where pad=1) union (select * from a_order_no_shard where pad=1) order by name limit 10
(select name as sort_a from aly_test where pad=1) union (select name from a_order_no_shard where pad=1) order by sort_a limit 10
(select name as sort_a,pad from aly_test where pad=1) union (select name,pad from a_order_no_shard where pad=1) order by sort_a,pad limit 10
#
#clear tables
#
drop table if exists aly_test
