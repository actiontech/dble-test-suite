...
select * from a_test a,a_order_no_shard b
select * from (select * from a_test where id<3) a,(select * from a_order_no_shard where id>3) b
select a.id,b.id,b.pad,a.t_id from a_test a,(select a_manager.id,a_manager.pad from a_test join a_manager where a_test.pad=a_manager.pad) b,(select * from a_order_no_shard where id>3) c where a.pad=b.pad and c.pad=b.pad
#
#join table
#
select * from a_test a join a_order_no_shard as b order by a.id,b.id
select * from a_test a inner join a_order_no_shard b order by a.id,b.id
select * from a_test a cross join a_order_no_shard b order by a.id,b.id
select a.id,a.name,a.pad,b.name from a_test a straight_join a_order_no_shard b on a.pad=b.pad
#
#SELECT ... UNION [ALL | DISTINCT] SELECT ... [UNION [ALL | DISTINCT] SELECT ...]
#
select * from a_test union all select * from a_manager union all select * from a_order_no_shard
select * from a_test union distinct select * from a_manager union distinct select * from a_order_no_shard
(select name from a_test where pad=1 order by id limit 10) union all (select name from a_order_no_shard where pad=1 order by id limit 10) order by name
(select name from a_test where pad=1 order by id limit 10) union distinct (select name from a_order_no_shard where pad=1 order by id limit 10) order by name
(select * from a_test where pad=1) union (select * from a_order_no_shard where pad=1) order by name limit 10
