...
select (select name from a_test order by id limit 1),id from a_order order by id
...
select a.pad,b.pad,count(*) from (select pad,count(id) t from a_test group by pad having t>1)a join (select pad,count(id) t from a_order group by pad order by pad)b group by b.pad order by b.pad
...
#order by(subquery)
#
select * from a_test order by (select pad from a_order order by id limit 1)
