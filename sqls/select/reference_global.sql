...
select * from test_global a,global_table2 b
select * from (select * from test_global where id<3) a,(select * from global_table2 where id>3) b
...
#join table
#
select * from test_global a join global_table2 as b order by a.id,b.id
select * from test_global a inner join global_table2 b order by a.id,b.id
select * from test_global a cross join global_table2 b order by a.id,b.id
select a.id,a.name,a.pad,b.name from test_global a straight_join global_table2 b on a.pad=b.pad
#
#SELECT ... UNION [ALL | DISTINCT] SELECT ... [UNION [ALL | DISTINCT] SELECT ...]
#
select * from test_global union all select * from global_table3 union all select * from global_table2
select * from test_global union distinct select * from global_table3 union distinct select * from global_table2
...
(select * from test_global where pad=1) union (select * from global_table2 where pad=1) order by id limit 10
