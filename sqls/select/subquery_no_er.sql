...
select a.id,b.id,b.pad,a.t_id from aly_test a,(select all * from a_two) b where a.t_id=b.o_id;
select a.id,b.id,b.pad,a.t_id from aly_test a,(select distinct * from a_two) b where a.t_id=b.o_id;
select * from (select * from a_two a group by a.id) a;
select * from (select pad,count(*) from a_two a group by pad) a;
select a.id,b.id,b.pad,a.t_id from aly_test a,(select * from a_two having pad>3) b where a.t_id=b.o_id;
select a.id,b.id,b.pad,a.t_id from aly_test a,(select * from a_two where pad>3 order by id) b where a.t_id=b.o_id;
select a.id,b.id,b.pad,a.t_id from aly_test a,(select * from a_two order by id limit 3) b where a.t_id=b.o_id;
select a.id,b.id,b.pad,a.t_id from aly_test a,(select * from a_two order by id limit 3) b where a.t_id=b.o_id limit 2;
select a.id,b.id,b.pad,a.t_id from aly_test a,(select * from a_two where pad>3) b where a.t_id=b.o_id;
select * from (select a_two.pad from aly_test left join a_two on aly_test.pad=a_two.pad) a;
select * from (select * from aly_test union select * from a_two) a where a.id >3;
select id,pad from aly_test where pad=(select min(id) from a_two);
...
select * from aly_test where pad<(select pad from a_two where id=3);
select * from aly_test having pad<(select pad from a_two where id=3);
select a.id,b.id,b.pad,a.t_id from aly_test a,(select * from a_two where pad>3) b where a.t_id=b.o_id;
select id,name,(select count(*) from a_two) count from aly_test;
select * from aly_test where pad like (select pad from a_two where id=3);
...
select * from aly_test,(select * from aly_test where id>3 union select * from a_two where id<2) a where a.id >3 and aly_test.pad=a.pad;
select count(*) from (select * from aly_test where pad=(select pad from a_two where id=1)) a;
#
#Second supplement
#
select (select name from aly_test order by id limit 1)
select * from aly_test where 'test_2'=(select name from a_two where id=2)
select * from aly_test where 5=(select count(*) from a_two)
select * from aly_test where 'test_2' like(select name from a_two where id=2)
...
select * from aly_test where (id,pad)=(select id,pad from a_two limit 1)
select * from aly_test where row(id,pad)=(select id,pad from a_two limit 1)
select id,name,pad from aly_test where (id,pad)in(select id,pad from a_two)
select id,name,pad from aly_test where (1,1)in(select id,pad from a_two)
SELECT pad FROM aly_test AS x WHERE x.id = (SELECT pad FROM a_two AS y WHERE x.id = (SELECT pad FROM a_three WHERE y.id = a_three.id))
