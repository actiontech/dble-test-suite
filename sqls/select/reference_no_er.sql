...
select * from aly_test a,a_two b
select * from (select * from aly_test where id<3) a,(select * from a_two where id>3) b
select a.id,b.id,b.pad,a.t_id from aly_test a,(select a_three.id,a_three.pad from aly_test join a_three where aly_test.pad=a_three.pad) b,(select * from a_two where id>3) c where a.pad=b.pad and c.pad=b.pad
#
#join table
#
select * from aly_test a join a_two as b order by a.id,b.id
select * from aly_test a inner join a_two b order by a.id,b.id
select * from aly_test a cross join a_two b order by a.id,b.id
select a.id,a.name,a.pad,b.name from aly_test a straight_join a_two b on a.pad=b.pad
#
#SELECT ... UNION [ALL | DISTINCT] SELECT ... [UNION [ALL | DISTINCT] SELECT ...]
#
select * from aly_test union all select * from a_three union all select * from a_two
select * from aly_test union distinct select * from a_three union distinct select * from a_two
(select name from aly_test where pad=1 order by id limit 10) union all (select name from a_two where pad=1 order by id limit 10) order by name
(select name from aly_test where pad=1 order by id limit 10) union distinct (select name from a_two where pad=1 order by id limit 10) order by name
(select * from aly_test where pad=1) union (select * from a_two where pad=1) order by name limit 10
(select name as sort_a from aly_test where pad=1) union (select name from a_two where pad=1) order by sort_a limit 10
(select name as sort_a,pad from aly_test where pad=1) union (select name,pad from a_two where pad=1) order by sort_a,pad limit 10
#
#clear tables
#
drop table if exists aly_test
