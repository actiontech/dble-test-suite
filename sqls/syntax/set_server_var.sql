...
select @@innodb_table_locks
#
#innodb_tmpdir: Used to define an alternate directory for temporary sort files created during online ALTER TABLE operations that rebuild the table.
#
...
#!share_conn
drop table if exists mytest_global1
create table mytest_global1(id int primary key auto_increment,data varchar(10))
insert into mytest_global1 (data) values ('aaa')
select id,data from mytest_global1 where id is null
set @@sql_auto_is_null=on
select @@sql_auto_is_null
insert into mytest_global1 (data) values ('bbb')
select id,data from mytest_global1 where id is null
