...
CREATE TABLE `global_table1` ( `c` json DEFAULT NULL, `g` int(11) DEFAULT NULL) ENGINE=InnoDB DEFAULT CHARSET=latin1
insert into global_table1 values ('{"id": "3", "name": "Barney"}' ,3),('{"id": "4", "name": "Betty"}' ,3), ('{"id": "2", "name": "Wilma"}',2)
#SELECT c, c->"$.id", g, n FROM global_table1 WHERE JSON_EXTRACT(c, "$.id") > 1 ORDER BY c->"$.name"
#SELECT c, c->"$.id", g FROM global_table1 WHERE c->"$.id" > 1 ORDER BY c->"$.name"
ALTER TABLE global_table1 ADD COLUMN n INT;
UPDATE global_table1 SET n=1 WHERE c->"$.id" = "4"
#SELECT c, c->"$.id", g, n FROM global_table1 WHERE JSON_EXTRACT(c, "$.id") > 1 ORDER BY c->"$.name"
DELETE FROM global_table1 WHERE c->"$.id" = "4"
#SELECT c->>'$.name' AS name FROM global_table1 WHERE g > 2
drop table if exists global_table1
