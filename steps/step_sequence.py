...
def step_impl(context,auto_table,data_type):
    result = None
    errMes = None
    context.mycat_conn = QueryDB(context.mycat.ip, context.mycat.user, context.mycat.passwd, "mytest",
                                 context.mycat.port, context)
    context.mycat_conn.dropTable(auto_table, context)
    sql = "show tables"
    result,errMes = context.mycat_conn.execute_sql(sql)
    ...
    context.mycat_conn.createTable(sql)
...
def step_impl(context, seq_type, auto_table, auto_col, rows):
    ...
    context.mycat_conn.execute_sql(sql)
    ...
    if type(result) == tuple:
        for i in range(len(result)):
            ...
            res, err = context.mycat_conn.execute_sql(sql)
...
def step_impl(context):
    ...
    if type(result) == tuple:
        for i in range(len(result)):
            ...
            res, err = context.dble_conn.execute_sql(sql)
            assert_that(int(res[0][0]), greater_than_or_equal_to(200))
...
def create_dnode_conn(context, dnode):
    ...
    conn = QueryDB(cur_dnode.mysql_ip, cur_dnode.mysql_user, cur_dnode.mysql_passwd, cur_dnode.mysql_db,
