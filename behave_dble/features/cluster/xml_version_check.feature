...
Feature: test "check xml version warning message in dble.log and dryrun"
  ...
  Scenario: check xml version warning  message in dryrun and dble.log when the version number：x.y is incorrect #2
    Given add attribute "{"version":"2.0"}" to rootnode in "server.xml"
    Given add attribute "{"version":"1.1"}" to rootnode in "rule.xml"
    Then execute admin cmd "reload @@config_all"
    Given sleep "2" seconds
    Then check following " " exist in file "/opt/dble/logs/dble.log" in "dble-1"
      """
      The dble-config-version is [0-9].[0-9],but the server.xml version is 2.0.There must be some incompatible config between two versions, please check it
      The dble-config-version is [0-9].[0-9],but the rule.xml version is 1.1.There might be some incompatible config between two versions, please check it
      """
    Then check following "not" exist in file "/opt/dble/logs/dble.log" in "dble-1"
      """
      The dble-config-version is [0-9].[0-9],but the schema.xml version
    ...
    Then check resultset "dryrun_rs" has lines with following column values
      | TYPE-0 | LEVEL-1 | DETAIL-2                                                                                                                                                    |
      | Xml    | NOTICE | The dble-config-version is [0-9].[0-9],but the rule.xml version is 1.1.There might be some incompatible config between two versions, please check it             |
      | Xml    | NOTICE | The dble-config-version is [0-9].[0-9],but the server.xml version is 2.0.There must be some incompatible config between two versions, please check it |
    ...
    Given add attribute "{"version":"1.0"}" to rootnode in "rule.xml"
    Then execute admin cmd "reload @@config_all"
    Given sleep "2" seconds
    Then stop dble in "dble-1"
    Given sleep "60" seconds
