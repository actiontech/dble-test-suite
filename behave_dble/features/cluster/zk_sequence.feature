...
Feature: Functional testing of global sequence generated by distributed timestamp
  ...
  #  4.START_TIME+17 years<the time of dble start
    ...
    """
        <table name="test_auto" shardingNode="dn1,dn2,dn3,dn4" incrementColumn="id" rule="hash-four"/>
    """
   ...
   Given update file content "{install_dir}/dble/conf/bootstrap.cnf" in "dble-1" with sed cmds
    ...
    #wait zk remove information about dble-1, dble与zk的session超时后（最小为tickTime=2s，最大为tickTime*20=2s*20），由zk去做清理，
    #循环检测zk中的online值,1代表dble-1节点
    Then check zk has "Y" the following values in "/dble/cluster-1/online" with retry "20,2" times in "dble-1"
    """
    \[2, 3\]
    """
    ...
    Then start dble in "dble-1" failed for
    """
    sequenceStartTime in cluster.cnf mustn't be over than dble start time, you can use default value 2010-11-04 09:42:54
    """
