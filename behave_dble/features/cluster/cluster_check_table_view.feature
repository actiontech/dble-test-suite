...
Feature: because 3.20.07 version change, the cluster function changes ,from doc: https://github.com/actiontech/dble-docs-cn/blob/master/2.Function/2.08_cluster.md
  ...
  Scenario: create view，alter view，drop view could success on shardingtable  #1
    ...
    Then execute sql in "dble-1" in "user" mode
      ...
      | conn_1 | false   | drop table if exists sharing_4_t1                | success   | schema1 |
      | conn_1 | true    | create table sharing_4_t1 (id int,name char(5))  | success   | schema1 |
    ...
    Then execute "user" cmd  in "dble-1" at background
      | conn   | toClose | sql                                                    | db      |
      | conn_1 | True    | create view view_test as select * from sharing_4_t1    | schema1 |
    ...
    Then execute sql in "dble-2" in "user" mode
      | conn   | toClose | sql                                                    | expect                                                                                  | db      |
      | conn_2 | true    | create view view_test as select * from sharing_4_t1    | other session/dble instance is operating view, try it later or check the cluster lock   | schema1 |
    Then execute sql in "dble-3" in "user" mode
      | conn   | toClose | sql                                                    | expect                                                                                  | db      |
      | conn_3 | true    | create view view_test as select * from sharing_4_t1    | other session/dble instance is operating view, try it later or check the cluster lock   | schema1 |
    ...
    Then execute sql in "dble-2" in "user" mode
      | conn   | toClose | sql                                                    | expect                             | db      |
      | conn_2 | false   | create view view_test as select * from sharing_4_t1    | Table 'view_test' already exists   | schema1 |
    Then execute sql in "dble-3" in "user" mode
      | conn   | toClose | sql                                                    | expect                             | db      |
      | conn_3 | false   | create view view_test as select * from sharing_4_t1    | Table 'view_test' already exists   | schema1 |
    ...
    Then check resultset "Res_A" has lines with following column values
      ...
      | view_test | create view view_test as  select * from sharing_4_t1  | latin1                 | latin1_swedish_ci      |
    ...
    Then check resultsets "Res_C" and "Res_B" are same in following columns
     ...
     Then execute sql in "dble-1" in "user" mode
      | conn   | toClose | sql                                                               | expect    | db      |
      | conn_1 | true    | insert into sharing_4_t1 values (1,1),(2,null),(3,'a'),(4,'aa')   | success   | schema1 |
    Then execute sql in "dble-3" in "user" mode
      | conn   | toClose  | sql                                                                | expect       | db      |
      | conn_3 | false    | select * from view_test where id=1                                 | length{(1)}  | schema1 |
      | conn_3 | true     | insert into sharing_4_t1 values (1,1),(2,null),(3,'a'),(4,'aa')    | success      | schema1 |
     Then execute sql in "dble-2" in "user" mode
      | conn   | toClose | sql                                                              | expect        | db      |
      | conn_2 | false   | select * from view_test                                          | length{(8)}   | schema1 |
      | conn_2 | false   | select * from view_test where id=1                               | length{(2)}   | schema1 |
      | conn_2 | true    | alter view view_test as select * from sharing_4_t1 where id =1   | success       | schema1 |
    ...
    Then check resultset "Res_A" has lines with following column values
      | View-0    | Create View-1                                                    | character_set_client-2 | collation_connection-3 |
      | view_test | create view view_test as  select * from sharing_4_t1 where id =1 | latin1                 | latin1_swedish_ci      |
...
#      | conn_1 | false   | show create view view_test            | Table 'schema1.view_test' doesn't exist   | schema1 |
      | conn_1 | true    | drop table if exists sharing_4_t1     | success                                   | schema1 |
