...
Feature: test flow_control about complex query
  ...
  Scenario: test flow_control about complex query   # 1
    ...
    Then execute sql in "dble-1" in "user" mode
      ...
      | conn_1 | False   | create table sharding_4_t1 (id int,a varchar(120) ,b varchar(120) ,c varchar(120) ,d varchar(120) ) default charset=utf8                                             | success | schema1 | utf8mb4 |
      ...
      | conn_1 | False   | insert into sharding_4_t1 values (1,repeat("中",32),repeat("华",32),repeat("民",32),repeat("国",32)),(2,repeat("中",32),repeat("华",32),repeat("民",32),repeat("国",32)) | success | schema1 | utf8mb4 |
    ...
    Then execute sql in "dble-1" in "user" mode
      | conn   | toClose | sql                                                   | expect  | db      | charset |
      | conn_1 | False   | drop view if exists view1                             | success | schema1 | utf8mb4 |
      | conn_1 | False   | drop view if exists view2                             | success | schema1 | utf8mb4 |
      | conn_1 | False   | create view view1 as select * from sharding_2_t1      | success | schema1 | utf8mb4 |
    ...
    Given execute sqls in "dble-1" at background
      ...
      | conn_2  | false   | select * from view1 join view2      | schema1 | utf8mb4 |
    # 9066 execute cmd
...
#    Given sleep "5" seconds
    ...
    Given execute sqls in "dble-1" at background
      | conn    | toClose | sql                                        | db      | charset |
      | conn_3  | false   | select * from (select * from view1) a      | schema1 | utf8mb4 |
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql                                                                                     | expect  | db               |
      | conn_0 | False   | select conn_send_task_queue,sql from session_connections                                | success | dble_information |
      | conn_0 | False   | flow_control @@list                                                                     | success | dble_information |
      | conn_0 | False   | flow_control @@set enableFlowControl=true flowControlStart=2 flowControlEnd=1           | success | dble_information |
...
#    Given sleep "5" seconds
    ...
    Given execute sqls in "dble-1" at background
      | conn    | toClose | sql                                                                    | db      | charset |
      | conn_4  | false   | select * from sharding_2_t1 a join sharding_4_t1  b on a.id = b.id     | schema1 | utf8mb4 |
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql                                                                                     | expect  | db               |
      | conn_0 | False   | select conn_send_task_queue,sql from session_connections                                | success | dble_information |
      | conn_0 | False   | flow_control @@list                                                                     | success | dble_information |
...
#    Given sleep "5" seconds
    ...
    Given execute sqls in "dble-1" at background
      | conn    | toClose | sql                                                                                     | db      | charset |
      | conn_5  | false   | select * from sharding_2_t1 where id in (select id from sharding_4_t1) order by id      | schema1 | utf8mb4 |
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql                                                                                     | expect  | db               |
      | conn_0 | False   | select conn_send_task_queue,sql from session_connections                                | success | dble_information |
      | conn_0 | False   | flow_control @@list                                                                     | success | dble_information |
    ...
    Given execute sqls in "dble-1" at background
      | conn    | toClose | sql                                                | db      | charset |
      | conn_6  | false   | select * from (select * from sharding_2_t1) a      | schema1 | utf8mb4 |
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql                                                                                     | expect  | db               |
      | conn_0 | False   | select conn_send_task_queue,sql from session_connections                                | success | dble_information |
      | conn_0 | False   | flow_control @@list                                                                     | success | dble_information |
    ...
    Given execute sqls in "dble-1" at background
      | conn    | toClose | sql                                                                       | db      | charset |
      | conn_7  | false   | select * from sharding_2_t1 union all select * from sharding_4_t1         | schema1 | utf8mb4 |
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql                                                                                     | expect  | db               |
      | conn_0 | False   | select conn_send_task_queue,sql from session_connections                                | success | dble_information |
      | conn_0 | False   | flow_control @@list                                                                     | success | dble_information |
...
#    Given sleep "2" seconds
    ...
    Given execute sqls in "dble-1" at background
      | conn    | toClose | sql                                                          | db      | charset |
      | conn_8  | false   | select * from sharding_2_t1 order by id limit 1000000000     | schema1 | utf8mb4 |
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql                                                                                     | expect  | db               |
      | conn_0 | False   | select conn_send_task_queue,sql from session_connections                                | success | dble_information |
      | conn_0 | False   | flow_control @@list                                                                     | success | dble_information |
