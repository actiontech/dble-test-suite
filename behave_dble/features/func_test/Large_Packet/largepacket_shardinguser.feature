...
#    """
    ...
    ##### /*!dble:dataNode=dn1*/insert into sharding_4_t1(id,c) values (7,"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
    Given update file content "/opt/SQLContext.py" in "dble-1" with sed cmds
      """
      s/insert into {0}({1},{2}) values ({3},/\/*!dble:dataNode=dn1*\/insert into {0}({1},{2}) values ({3},/g
      """
...
#      """
    ...
    Given update file content "/opt/SQLContext.py" in "dble-1" with sed cmds
      """
      s/dble:dataNode=dn1/dble:sql=select c from tb1/g
      s/insert into {0}({1},{2}) values ({3},/update {0} set {1}=/g
      s/.format(self.table, cols_keys, target_col_key, cols_values)/.format(self.table,target_col_key)/g
