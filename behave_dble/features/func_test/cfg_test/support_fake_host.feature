...
Feature: db.xml support fake host
  ...
  Scenario: The dbGroup have fake write dbInstance and write read dbInstance #7
    ...
    Then execute sql in "dble-1" in "user" mode
      ...
      | conn_0 | false   | update sharding_4_t1 set name='test' where id=3   | the dbInstance[ha_group2:hostM2:127.0.0.1:8066] is fake node. Please check the dbInstance whether or not it is used | schema1 ||
      | conn_0 | false   | delete from sharding_4_t1 where id>2              | the dbInstance[ha_group2:hostM2:127.0.0.1:8066] is fake node. Please check the dbInstance whether or not it is used | schema1 ||
      | conn_0 | true    | drop table if exists sharding_4_t1                | the dbInstance[ha_group2:hostM2:127.0.0.1:8066] is fake node. Please check the dbInstance whether or not it is used | schema1 ||
