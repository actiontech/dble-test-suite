...
Feature: test config in user.xml  ---  analysisUser
  ...
  Scenario: analysisUser user supporte management cmd success    #13
    ...
    Then execute sql in "dble-1" in "admin" mode
      ...
      | conn_0 | False   | select * from dble_db_group where name = 'ha_group3'          | has{(('ha_group3', 'select 1', 0, 1, 2, 100, 'false', 'true'),)}   | dble_information |
...
#########   show
      ...
      | conn_0 | False   | show @@sysparam                                                   | length{(120)} |
