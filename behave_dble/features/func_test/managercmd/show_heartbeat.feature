...
# set rwSplitMode=0 ,one slave disabled="true"
    ...
    <dbGroup rwSplitMode="0" name="ha_group2" delayThreshold="100" >
        ...
        <dbInstance name="hostS2" password="111111" url="172.100.9.3:3307" user="test" maxCon="1000" minCon="10" disabled="true"/>
    </dbGroup>
    """
...
#case one slave is disable then check RS_CODE is "init",one slave is enable then check RS_CODE is "ok"
    ...
    Then check resultset "11" has lines with following column values
      ...
      | hostM1 | 172.100.9.5 | 3307   | ok        | 0       | idle     | 0         | false  | None          |
      | hostM2 | 172.100.9.6 | 3307   | ok        | 0       | idle     | 5000      | false  | None          |
      | hostS1 | 172.100.9.2 | 3307   | ok        | 0       | idle     | 5000      | false  | None          |
      | hostS2 | 172.100.9.3 | 3307   | init      | 0       | idle     | 5000      | true   | None          |
#case one slave is down ,check master RS_CODE is "ok" and slave RS_CODE is "error"
    ...
    Then check resultset "12" has lines with following column values
      ...
      | hostM1 | 172.100.9.5 | 3307   | ok        | 0       | idle     | 0         | false  | None                                                                                                                           |
      | hostM2 | 172.100.9.6 | 3307   | ok        | 0       | idle     | 5000      | false  | None                                                                                                                           |
      | hostS1 | 172.100.9.2 | 3307   | error     | 0       | idle     | 5000      | false  | connection Error//heartbeat conn for sql[/*# from=1 reason=heartbeat*/show slave status] is closed, due to abnormal connection |
      | hostS2 | 172.100.9.3 | 3307   | init      | 0       | idle     | 5000      | true   | None                                                                                                                           |
...
#case one slave set iptables to check slave RS_CODE is "time_out"
    ...
    Then check resultset "16" has lines with following column values
      ...
      | hostM1 | 172.100.9.5 | 3307   | ok        | 0       | 0         | false  | None          |
      | hostM2 | 172.100.9.6 | 3307   | ok        | 0       | 5000      | false  | None          |
      | hostS1 | 172.100.9.2 | 3307   | time_out  | 0       | 5000      | false  | None          |
      | hostS2 | 172.100.9.3 | 3307   | init      | 0       | 5000      | true   | None          |
    ...
    Then check resultset "17" has lines with following column values
      ...
      | hostM1 | 172.100.9.5 | 3307   | ok        | 0       | 0         | false  | None           |
      | hostM2 | 172.100.9.6 | 3307   | ok        | 0       | 5000      | false  | None           |
      | hostS1 | 172.100.9.2 | 3307   | ok        | 0       | 5000      | false  | None           |
      | hostS2 | 172.100.9.3 | 3307   | init      | 0       | 5000      | true   | None           |
...
#check slave is "error" and RETRY is equal errorRetryCount=2 DBLE0REQ-633
    Then check resultset "21" has lines with following column values
      | NAME-0 | HOST-1      | PORT-2 | RS_CODE-3 | RETRY-4 | STATUS-5 | TIMEOUT-6  | STOP-9 | RS_MESSAGE-10                                                                                                                   |
      | hostM2 | 172.100.9.6 | 3307   | ok        | 0       | idle     | 10000      | false  | None                                                                                                                            |
      | hostS1 | 172.100.9.2 | 3307   | error     | 2       | idle     | 10000      | false  | connection Error//heartbeat conn for sql[/*# from=1 reason=heartbeat*/show slave status] is closed, due to abnormal connection  |
    Given start mysql in host "mysql-slave1"
    ...
    Then check resultset "22" has lines with following column values
      | NAME-0 | HOST-1      | PORT-2 | RS_CODE-3 | RETRY-4 | STATUS-5 | TIMEOUT-6  | STOP-9 | RS_MESSAGE-10    |
      | hostM2 | 172.100.9.6 | 3307   | ok        | 0       | idle     | 10000      | false  | None             |
      | hostS1 | 172.100.9.2 | 3307   | ok        | 0       | idle     | 10000      | false  | None             |
...
# case 3:add new slave and down old slave,check salve RS_CODE
    ...
    Then check resultset "31" has lines with following column values
      | NAME-0 | HOST-1      | PORT-2 | RS_CODE-3 | RETRY-4 | STATUS-5 | TIMEOUT-6 | STOP-9 | RS_MESSAGE-10                                                                                                                  |
      | hostM2 | 172.100.9.6 | 3307   | ok        | 0       | idle     | 10000     | false  | None                                                                                                                           |
      | hostS1 | 172.100.9.2 | 3307   | error     | 0       | idle     | 10000     | false  | connection Error//heartbeat conn for sql[/*# from=1 reason=heartbeat*/show slave status] is closed, due to abnormal connection |
      | hostS2 | 172.100.9.3 | 3307   | ok        | 0       | idle     | 10000     | false  | None                                                                                                                           |
    Given start mysql in host "mysql-slave1"
    Given sleep "10" seconds
...
# case 4 :set master down to check RS_CODE
    ...
    Then check resultset "41" has lines with following column values
      ...
      | hostM2 | 172.100.9.6 | 3307   | error     | 0       | idle     | 10000     | false  |
      | hostS1 | 172.100.9.2 | 3307   | ok        | 0       | idle     | 10000     | false  |
      | hostS2 | 172.100.9.3 | 3307   | ok        | 0       | idle     | 10000     | false  |
...
#case set master iptables to check master RS_CODE is "time_out"
    ...
    Then check resultset "43" has lines with following column values
      | NAME-0 | HOST-1      | PORT-2 | RS_CODE-3 | RETRY-4 | TIMEOUT-6 | STOP-9 | RS_MESSAGE-10 |
      | hostM2 | 172.100.9.6 | 3307   | time_out  | 0       | 10000     | false  | None          |
      | hostS1 | 172.100.9.2 | 3307   | ok        | 0       | 10000     | false  | None          |
      | hostS2 | 172.100.9.3 | 3307   | ok        | 0       | 10000     | false  | None          |
    Given execute oscmd in "mysql-master2"
    """
    iptables -F
    """
    ...
    Then check resultset "44" has lines with following column values
      | NAME-0 | HOST-1      | PORT-2 | RS_CODE-3 | RETRY-4 | TIMEOUT-6 | STOP-9 | RS_MESSAGE-10 |
      | hostM2 | 172.100.9.6 | 3307   | ok        | 0       | 10000     | false  | None          |
      | hostS1 | 172.100.9.2 | 3307   | ok        | 0       | 10000     | false  | None          |
      | hostS2 | 172.100.9.3 | 3307   | ok        | 0       | 10000     | false  | None          |
    Then execute admin cmd "reload @@config"
