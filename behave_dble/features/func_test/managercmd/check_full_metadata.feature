...
Feature: test "check full @@metadata...'"
  ...
  Scenario: backend tables in datanode are inconsistent or lack in some datanode for a config sharding/global table, check metadata and query #5
    ...
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql                                                           | expect            |
      | conn_1 | False   | check full @@metadata where consistent_in_data_nodes =0       | hasStr{`test4`}   |
      | conn_1 | False   | check full @@metadata where consistent_in_data_nodes =0       | hasNoStr{`test2`} |
      | conn_1 | False   | check full @@metadata where consistent_in_data_nodes =1       | hasStr{`test2`}   |
      | conn_1 | False   | check full @@metadata where consistent_in_data_nodes =1       | hasNoStr{`test4`} |
    ...
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql                                                           | expect            |
      | conn_1 | False   | check full @@metadata where consistent_in_data_nodes =0       | hasStr{`test5`}   |
      | conn_1 | False   | check full @@metadata where consistent_in_data_nodes =0       | hasNoStr{`test2`} |
      | conn_1 | False   | check full @@metadata where consistent_in_data_nodes =1       | hasStr{`test2`}   |
      | conn_1 | False   | check full @@metadata where consistent_in_data_nodes =1       | hasNoStr{`test5`} |
      | conn_1 | False   | check full @@metadata where schema='schema1' and table='test2'| hasNoStr{`name`}  |
      | conn_1 | False   | check full @@metadata where schema='schema1' and table='test2'| hasStr{`age`}     |
      | conn_1 | True    | check full @@metadata where schema='schema1' and table='test6'| hasNoStr{`id`}    |
    Then execute sql in "dble-1" in "user" mode
  ...
  Scenario: Some of datahost's writehost(with or without readhost) cannot be connectted, check metadata and query #6
    ...
    <dataHost balance="0" maxCon="1000" minCon="10" name="ha_group2" slaveThreshold="-1" >
        ...
        <readHost host="hostS2" url="172.100.9.6:3307" password="111111" user="test"/>
        </writeHost>
    </dataHost>
    """
    Then execute admin cmd "reload @@config_all"
  ...
  Scenario: meta data check should ignore AUTO_INCREMENT difference, check matadata、rload and dble.log #8
    ...
    Then execute sql in "dble-1" in "admin" mode
      | sql                                                     | expect                          |
      | check full @@metadata where consistent_in_data_nodes =0 | hasNoStr{`mytest_auto_test1`}   |
  ...
  Scenario: meta data check has DEFAULT CHARSET, check mconsistent_in_memory #10
    ...
    """
      <system>
        <property name="checkTableConsistency">1</property>
        <property name="checkTableConsistencyPeriod">100</property>
      </system>
     """
    Given Restart dble in "dble-1" success
    Then execute sql in "dble-1" in "user" mode
    ...
    Then execute sql in "dble-1" in "admin" mode
      | sql                                                      | expect                          |
      | check full @@metadata where consistent_in_data_nodes = 0 | hasNoStr{`sharding_2_t1`}       |
      | check full @@metadata where consistent_in_data_nodes = 0 | hasNoStr{`sharding_4_t1`}       |
      | check full @@metadata where consistent_in_data_nodes = 1 | hasStr{`sharding_2_t1`}         |
      | check full @@metadata where consistent_in_data_nodes = 1 | hasStr{`sharding_4_t1`}         |
    Then execute admin cmd "reload @@metadata"
    Then execute sql in "dble-1" in "admin" mode
      | sql                                                      | expect                          |
      | check full @@metadata where consistent_in_data_nodes = 0 | hasNoStr{`sharding_2_t1`}       |
