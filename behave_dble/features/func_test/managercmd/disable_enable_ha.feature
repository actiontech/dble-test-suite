...
#    The expect fail msg is tmp,for github issue:#1528
    Then execute sql in "dble-1" in "user" mode
        | user | passwd | conn   | toClose  | sql                                        | expect              | db       |
        | test | 111111 | conn_0 | true     | insert into sharding_4_t1 values(1),(2)    | Connection{DataHost[172.100.9.6:3306],Schema[db1]} refused  |  schema1  |
    ...
    Then check exist xml node "{'tag':'dataHost/readHost','kv_map':{'host':'hostM2'}}" in "/opt/dble/conf/schema.xml"  in host "dble-1"
    Then check exist xml node "{'tag':'dataHost/writeHost','kv_map':{'host':'slave1'}}" in "/opt/dble/conf/schema.xml"  in host "dble-1"
    Then get resultset of admin cmd "show @@datasource" named "show_ds_rs"
    Then check resultset "show_ds_rs" has lines with following column values
    | DATAHOST-0 | NAME-1   | HOST-2        | PORT-3 | W/R-4  | ACTIVE-5 | DISABLED-11 |
    ...
    Then check exist xml node "{'tag':'dataHost/readHost','kv_map':{'host':'hostM2','disabled':'false'}}" in "/opt/dble/conf/schema.xml"  in host "dble-1"
