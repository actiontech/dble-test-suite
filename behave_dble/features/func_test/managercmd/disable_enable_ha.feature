...
#    check transaction is killed forcely
    Then execute sql in "dble-1" in "user" mode
      | conn   | toClose  | sql                         | expect                                                                                                        | db       |
      | conn_0 | true     | select * from sharding_4_t1 | java.io.IOException: the dbInstance[172.100.9.6:3307] is disable. Please check the dbInstance disable status  | schema1  |
    ...
    """
     <dbGroup rwSplitMode="0" name="ha_group2" delayThreshold="100" >
       <heartbeat>select user()</heartbeat>
        <dbInstance name="hostM2" password="111111" url="172.100.9.6:3307" user="test" maxCon="1000" minCon="10" primary="true" disabled="true">
        </dbInstance>
