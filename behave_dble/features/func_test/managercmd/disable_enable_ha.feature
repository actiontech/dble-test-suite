...
#    check transaction is killed forcely
    Then execute sql in "dble-1" in "user" mode
      | conn   | toClose  | sql                         | expect                                                                                                                         | db       |
      | conn_0 | true     | select * from sharding_4_t1 | java.io.IOException: the dbInstance[ha_group2:hostM2:172.100.9.6:3306] is disable. Please check the dbInstance disable status  | schema1  |
