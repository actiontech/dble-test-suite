...
Feature: reload @@config_all -sr
  ...
  Scenario: open transaction, and execute "reload @@config_all -sr" or "reload @@config_all -s -r", transaction closed successfully
    ...
    # 1 reload @@config_all -sr, donot do diff, rebuild backend conn, skip in use backend conn
    ...
    #2 reload @@config_all -sr: add dataNode, rebuild backend conn , add new backend conn
    Given delete the following xml segment
      | file       | parent         | child              |
      | schema.xml | {'tag':'root'} | {'tag':'dataNode'} |
      | schema.xml | {'tag':'root'} | {'tag':'dataHost'} |
    ...
    #3 Start the transaction, add readHost, Reload config_all -sr succeeds, the transaction connection is closed, and all connections are rebuilt
    Given delete the following xml segment
      | file       | parent         | child              |
      | schema.xml | {'tag':'root'} | {'tag':'dataHost'} |
    ...
    #4 Start the transaction, add readHost, Reload config_all -sr succeeds, the transaction connection is closed, and all connections are rebuilt
    Then execute sql in "dble-1" in "user" mode
      | user | passwd | conn   | toClose | sql                                              | expect  | db      |
      | test | 111111 | conn_0 | false   | drop table if exists sharding_4_t1               | success | schema1 |
      | test | 111111 | conn_0 | false   | create table sharding_4_t1 (id int)              | success | schema1 |
