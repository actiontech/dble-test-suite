...
Feature: test " check @@global schema = '' [and table = '']"
  ...
  Scenario: check @@global schema = '' [and table = '']  sql test    #1
  ...
  Scenario: check @@global schema = '' [and table = '']     #3
    """
  ...
  #case global table meta not exists
    ...
    Then check following text exist "Y" in file "/opt/dble/logs/dble.log" in host "dble-1" retry "12,2" times
    """
   ...
   #case global table meta exists
    ...
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql             | expect                                                      |
      | conn_0 | true    | check @@global  | has{(('schema1', 'g1', 1, 0),('schema2', 'g3', 1, 0),)}     |
   ...
   #case the global table one dataNode is not created, others dataNode are created and not the same
    ...
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql             | expect                                                      |
      | conn_0 | true    | check @@global  | has{(('schema1', 'g1', 3, 0),('schema2', 'g3', 2, 1),)}     |
   ...
   #case the mysql server is down
    ...
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql              | expect                                                      |
      | conn_0 | true    | check @@global   | has{(('schema1', 'g1', 1, 2),('schema2', 'g3', 1, 2),)}     |
    Given start mysql in host "mysql-master1"
    Given sleep "11" seconds
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql             | expect                                                      | timeout |
      | conn_0 | true    | check @@global  | has{(('schema1', 'g1', 3, 0),('schema2', 'g3', 2, 1),)}     | 5       |
    Then execute sql in "dble-1" in "user" mode
      | conn   | toClose  | sql                                  | expect  |
      | conn_1 | false    | drop table if exists schema1.g1      | success |
      | conn_1 | True     | drop table if exists schema2.g3      | success |