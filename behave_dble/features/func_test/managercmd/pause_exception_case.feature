...
Feature: execute pause in an exception scenario

  Scenario: resume datanodes that are not suspended traffic #1
  ...
  Scenario: execute the pause command multiple times #2
    Then execute sql in "dble-1" in "user" mode
      | user | passwd | conn   | toClose | sql                                                  | expect  | db      |
      | test | 111111 | conn_0 | True    | drop table if exists sharding_4_t1                   | success | schema1 |
      | test | 111111 | conn_0 | True    | create table sharding_4_t1 (id int,name varchar(20)) | success | schema1 |
  ...
  Scenario: execute the resume command before the pause command expires #3
    Then execute sql in "dble-1" in "user" mode
      | user | passwd | conn   | toClose | sql                                                  | expect  | db      |
      | test | 111111 | conn_0 | True    | drop table if exists sharding_4_t1                   | success | schema1 |
      | test | 111111 | conn_0 | True    | create table sharding_4_t1 (id int,name varchar(20)) | success | schema1 |
