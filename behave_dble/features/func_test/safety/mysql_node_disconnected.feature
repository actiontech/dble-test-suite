...
Feature: #mysql node disconnected,check the change of dble
  ...
  Scenario: # some of the backend nodes was disconnected   #2
    ...
    Then execute sql in "dble-1" in "user" mode
      ...
      | test | 111111 | conn_0 | True    | insert into test_table values(2,4)    | DataNode[172.100.9.5]'s init error, please check it can be connected. The current Node is {DataHost[172.100.9.5:3306,Schema[db2]}  | schema1 |
    Given start mysql in host "mysql-master1"
