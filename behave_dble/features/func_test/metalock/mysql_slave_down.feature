...
#       After the node is restored, the successful release of the lock is verified
  ...
  Scenario: mysql one node down     #1
    ...
    Then execute sql in "mysql-master1"
      | conn    | toClose | sql                         | expect                                                                                                                           | db  |
      | conn_11 | False   | desc sharding_4_t1          | has{(('id', 'int(11)', 'YES', '', None, ''), ('name', 'int(11)', 'YES', '', None, ''), ('age', 'int(11)', 'YES', '', None, ''))} | db1 |
      | conn_21 | False   | desc sharding_4_t1          | has{(('id', 'int(11)', 'YES', '', None, ''), ('name', 'int(11)', 'YES', '', None, ''), ('age', 'int(11)', 'YES', '', None, ''))} | db2 |
    Then execute sql in "dble-1" in "user" mode
      | conn   | toClose | sql                                    | expect       | db      |
