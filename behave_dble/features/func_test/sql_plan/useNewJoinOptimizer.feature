...
#more information find in confluence: http://10.186.18.11/confluence/pages/viewpage.action?pageId=32064447
  ...
  Scenario: shardingTable  + shardingTable  +  shardingTable                              #1
    ...
    #rule-B、C、D:left join & inner join & oner ER, ER first
     Given execute single sql in "dble-1" in "user" mode and save resultset in "rs_T"
       | conn    | toClose | sql                                                         | db|
       | conn_20 | false   | explain SELECT a.Name,a.DeptName,b.Manager,c.salary FROM Employee a LEFT JOIN  Level c on a.Level=c.levelname and a.empid=2242 INNER JOIN Dept b on a.DeptName= b.DeptName and c.salary=10000 and b.deptid=2 order by a.Name | schema1|
     Then check resultset "rs_T" has lines with following column values
    ...
    Then execute sql in "dble-1" and the result should be consistent with mysql
        | conn   | toClose | sql                                                         | db|
        | conn_20| true    | SELECT a.Name,a.DeptName,b.Manager,c.salary FROM Employee a LEFT JOIN  Level c on a.Level=c.levelname and a.empid=2242 INNER JOIN Dept b on a.DeptName= b.DeptName and c.salary=10000 and b.deptid=2  order by a.Name | schema1 |
