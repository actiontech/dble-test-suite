...
#    Given delete all backend mysql tables
  ...
  @delete_mysql_tables
   Scenario: GlobalTable  + [otherTypeTable]            #3
    ...
    Then execute sql in "dble-1" and the result should be consistent with mysql
     ...
     Then check resultset "J" has lines with following column values
          | SHARDING_NODE-0   | TYPE-1          | SQL/REF-2                                                                     |
          | dn1_0             | BASE SQL        | select `b`.`deptname`,`b`.`deptid`,`b`.`manager`,`a`.`name`,`a`.`empid`,`a`.`deptname`,`a`.`level` from  `Dept` `b` join  `Employee` `a` on `b`.`deptname` = `a`.`deptname` where 1=1  ORDER BY `a`.`name` ASC,`b`.`deptname` ASC |
          | dn2_0             | BASE SQL        | select `b`.`deptname`,`b`.`deptid`,`b`.`manager`,`a`.`name`,`a`.`empid`,`a`.`deptname`,`a`.`level` from  `Dept` `b` join  `Employee` `a` on `b`.`deptname` = `a`.`deptname` where 1=1  ORDER BY `a`.`name` ASC,`b`.`deptname` ASC |
          | merge_and_order_1 | MERGE_AND_ORDER | dn1_0; dn2_0                                                                                                                                                                                                                      |
          | shuffle_field_1   | SHUFFLE_FIELD   | merge_and_order_1                                                                                                                                                                                                                 |
  ...
  #        | conn_1  | true      | SELECT * FROM Employee a INNER JOIN Level c on a.level=c.levelname LEFT JOIN Dept b on a.DeptName= b.DeptName UNION SELECT * FROM Employee a LEFT JOIN Level c on a.level=c.levelname INNER JOIN Dept b on a.DeptName= b.DeptName  | schema1|
     ...
     Then check resultset "C" has lines with following column values
          | SHARDING_NODE-0   | TYPE-1          | SQL/REF-2                                                                     |
          | dn3_0             | BASE SQL        | select `c`.`name`,`c`.`age`,`c`.`country`,`c`.`deptname`,`b`.`deptname`,`b`.`deptid`,`b`.`manager`,`a`.`name`,`a`.`empid`,`a`.`deptname`,`a`.`level` from  (  `Info` `c` join  `Dept` `b` on `c`.`DeptName` = `b`.`DeptName` )  join  `Employee` `a` on `b`.`DeptName` = `a`.`DeptName` and `c`.`DeptName` = `a`.`DeptName` where 1=1  ORDER BY `c`.`age` ASC//select `c`.`name`,`c`.`age`,`c`.`country`,`c`.`deptname`,`a`.`name`,`a`.`empid`,`a`.`deptname`,`a`.`level`,`b`.`deptname`,`b`.`deptid`,`b`.`manager` from  (  `Info` `c` join  `Employee` `a` on `c`.`DeptName` = `a`.`DeptName` )  join  `Dept` `b` on `a`.`DeptName` = `b`.`DeptName` and `c`.`DeptName` = `b`.`DeptName` where 1=1  ORDER BY `c`.`age` ASC |
          | dn4_0             | BASE SQL        | select `c`.`name`,`c`.`age`,`c`.`country`,`c`.`deptname`,`b`.`deptname`,`b`.`deptid`,`b`.`manager`,`a`.`name`,`a`.`empid`,`a`.`deptname`,`a`.`level` from  (  `Info` `c` join  `Dept` `b` on `c`.`DeptName` = `b`.`DeptName` )  join  `Employee` `a` on `b`.`DeptName` = `a`.`DeptName` and `c`.`DeptName` = `a`.`DeptName` where 1=1  ORDER BY `c`.`age` ASC//select `c`.`name`,`c`.`age`,`c`.`country`,`c`.`deptname`,`a`.`name`,`a`.`empid`,`a`.`deptname`,`a`.`level`,`b`.`deptname`,`b`.`deptid`,`b`.`manager` from  (  `Info` `c` join  `Employee` `a` on `c`.`DeptName` = `a`.`DeptName` )  join  `Dept` `b` on `a`.`DeptName` = `b`.`DeptName` and `c`.`DeptName` = `b`.`DeptName` where 1=1  ORDER BY `c`.`age` ASC |
          | merge_and_order_1 | MERGE_AND_ORDER | dn3_0; dn4_0                                                                                                                                                                                                                                                                                                                                                  |
          | shuffle_field_1   | SHUFFLE_FIELD   | merge_and_order_1                                                                                                                                                                                                                                                                                                                                             |
          | dn1_0             | BASE SQL        | select `d`.`levelname`,`d`.`levelid`,`d`.`salary` from  `Level` `d` ORDER BY `d`.`levelid` ASC                                                                                                                                                                                                                                                                |
          | dn2_0             | BASE SQL        | select `d`.`levelname`,`d`.`levelid`,`d`.`salary` from  `Level` `d` ORDER BY `d`.`levelid` ASC                                                                                                                                                                                                                                                                |
