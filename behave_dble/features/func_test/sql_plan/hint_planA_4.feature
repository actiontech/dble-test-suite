...
Feature: test with hint plan A with other table type
 ...
 # 1.1 left join & left join & 1 ER  ab &ac
    ...
    Then check resultset "join_rs" has lines with following column values
      ...
      | dn5_0             | BASE SQL(May No Need) | HINT_NEST_LOOP - shuffle_field_1's RESULTS; select `c`.`name`,`c`.`age`,`c`.`country`,`c`.`deptname` from  `Info` `c` where `c`.`deptname` in ('{NEED_TO_REPLACE}') ORDER BY `c`.`deptname` ASC                         |
      | merge_1           | MERGE                 | dn5_0                                                                                                                                                                                                                   |
    ...
    # 1.2 left join & left join & 1 ER  ab & bc
    Given execute single sql in "dble-1" in "user" mode and save resultset in "join_rs"
      | conn   | toClose | sql                                                                                                                                                             | expect  | db      |
    ...
    Then check resultset "join_rs" has lines with following column values
      ...
      | dn5_0//dn5_1      | BASE SQL        | select `c`.`name`,`c`.`age`,`c`.`country`,`c`.`deptname` from  `Info` `c` order by `c`.`deptname` ASC     |
      | merge_2           | MERGE           | dn5_0//dn5_1                                                                                              |
