...
Feature: test with hint plan A with other table type
  ...
  """
    ...
    Then execute sql in "dble-1" in "user" mode
      ...
      | conn_1 | False   | insert into Dept values('Finance',2,'George'),('Sales',3,'Harriet'),('Market',4,'Tom') ,('Market',4,')                                                                                                                                                                                                                                                                | success | schema1 |
...
#
#
#         # 1.2 left join & left join & 1 ER  abc
#    Given execute single sql in "dble-1" in "user" mode and save resultset in "join_rs"
#      | conn   | toClose | sql                                                                                                                                                                                            | expect  | db      |
...
#      | merge_and_order_1 | MERGE_AND_ORDER       | dn3_0; dn4_0                                                                                                                                                       |
#      | shuffle_field_1   | SHUFFLE_FIELD         | merge_and_order_1                                                                                                                                                  |
#      | dn6_0             | BASE SQL(May No Need) | HINT_NEST_LOOP - shuffle_field_1's RESULTS; select `c`.`country`,`c`.`name` from  `Info` `c` where `c`.`name` in ('{NEED_TO_REPLACE}') ORDER BY `c`.`name` ASC     |
#      | merge_1           | MERGE                 | dn6_0                                                                                                                                                              |
#      | join_1            | JOIN                  | shuffle_field_1; merge_1                                                                                                                                           |
#      | order_1           | ORDER                 | join_1                                                                                                                                                             |
#      | shuffle_field_2   | SHUFFLE_FIELD         | order_1                                                                                                                                                            |
...
#      | conn_1 | true    | /*#dble:plan= (a,b) & c */ SELECT a.name,a.deptname,b.manager,c.country FROM Employee a LEFT JOIN Dept b ON a.deptname=b.deptname LEFT JOIN Info c ON b.manager=c.name ORDER BY a.name | schema1 |
    ...
    # 1.3 left join & left join & NO ER  abc
    ...
    Then check resultset "join_rs" has lines with following column values
      | SHARDING_NODE-0   | TYPE-1                | SQL/REF-2                                                                                                                                                      |
      | dn3_0             | BASE SQL              | select `a`.`name`,`a`.`deptname`,`b`.`manager` from  `Employee` `a` left join  `Dept` `b` on `a`.`name` = `b`.`manager` where 1=1  ORDER BY `b`.`manager` ASC  |
      | dn4_0             | BASE SQL              | select `a`.`name`,`a`.`deptname`,`b`.`manager` from  `Employee` `a` left join  `Dept` `b` on `a`.`name` = `b`.`manager` where 1=1  ORDER BY `b`.`manager` ASC  |
      | merge_and_order_1 | MERGE_AND_ORDER       | dn3_0; dn4_0                                                                                                                                                   |
      | shuffle_field_1   | SHUFFLE_FIELD         | merge_and_order_1                                                                                                                                              |
      | dn6_0             | BASE SQL(May No Need) | HINT_NEST_LOOP - shuffle_field_1's RESULTS; select `c`.`country`,`c`.`name` from  `Info` `c` where `c`.`name` in ('{NEED_TO_REPLACE}') ORDER BY `c`.`name` ASC |
      | merge_1           | MERGE                 | dn6_0                                                                                                                                                          |
