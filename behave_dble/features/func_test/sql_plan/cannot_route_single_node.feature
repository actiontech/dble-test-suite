...
Feature: following complex queries are not able to send one shardingnode
   ...
   Scenario: execute "explain sql" and check result
    ...
    Then execute sql in "mysql"
      | conn   | toClose | sql                                                                                                          | expect  | db      |
      | conn_1 | False   | drop table if exists sharding_two_node2                                                                      | success | schema1 |
      | conn_1 | False   | create table sharding_two_node2(`id` int(11) DEFAULT NULL,`name` char(20) COLLATE utf8mb4_bin DEFAULT NULL)  | success | schema1 |
      | conn_1 | False   | drop table if exists sharding_two_node                                                                       | success | schema1 |
      | conn_1 | False   | create table sharding_two_node(`id` int(11) DEFAULT NULL,`name` char(20) COLLATE utf8mb4_bin DEFAULT NULL)   | success | schema1 |
    ...
    Then execute sql in "dble-1" and the result should be consistent with mysql
      | conn   | toClose | sql                                                                                                                 | db      |
      | conn_1 | true    | select a.* from sharding_two_node2 a where a.id =2 or a.id in (select b.id from sharding_two_node b ) order by a.id | schema1 |
