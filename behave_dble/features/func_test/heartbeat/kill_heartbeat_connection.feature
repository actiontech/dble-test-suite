...
Feature: heartbeat basic test
  ...
  Scenario: heartbeat timeout test #2
    ...
    """
     <dbGroup rwSplitMode="0" name="ha_group1" delayThreshold="100">
        <heartbeat timeout="10">select user()</heartbeat>
    ...
    /fieldEofResponse/{:a;n;s/Thread.sleep([0-9]*L)/Thread.sleep(30000L)/;/\}/!ba}
    """
   ...
   Given execute linux command in "dble-1" and save result in "master1_heartbeat_id"
    ...
    """
    #sleep 5s to go into next heartbeat,then heartbeat will set as timeout, and the connect pool is not available
...
#      | show @@heartbeat |
  ...
  Scenario: heartbeat connection recover success in retry 'errorRetryCount' times, the heartbeat will set as Ok,and the connection pool is available in retry period #5
    ...
    """
    retry to do heartbeat for the 3 times
    heartbeat to \[172.100.9.5:3306\] setOk
    """
 ...
 @btrace
 Scenario: in retry period timeout the heartbeat will set as timeout and the connection pool is not available #6
    Given delete the following xml segment
      | file       | parent         | child              |
      | sharding.xml | {'tag':'root'} | {'tag':'schema'}   |
      | sharding.xml | {'tag':'root'} | {'tag':'shardingNode'} |
    ...
    """
    s/Thread.sleep([0-9]*L)/Thread.sleep(1L)/
    /fieldEofResponse/{:a;n;s/Thread.sleep([0-9]*L)/Thread.sleep(600000L)/;/\}/!ba}
    """
   Given prepare a thread run btrace script "BtraceHeartbeat.java" in "dble-1"
   Given execute linux command in "dble-1" and save result in "master1_heartbeat_id1"
    """
    mysql -P{node:manager_port} -u{node:manager_user} -e "show @@backend" | awk '{print $3, $NF}' | grep true | awk '{print $1}'
    """
    Given kill mysql conns in "mysql-master1" in "master1_heartbeat_id1"
    Then check btrace "BtraceHeartbeat.java" output in "dble-1"
    """
    before fieldEofResponse
    """
   ...
   Given delete file "/opt/dble/BtraceHeartbeat.java" on "dble-1"
   Given delete file "/opt/dble/BtraceHeartbeat.java.log" on "dble-1"
   #sleep 10s to let heartbeat timeout
   Given sleep "10" seconds
   Given execute linux command in "dble-1" and save result in "master1_heartbeat_id2"
    """
    mysql -P{node:manager_port} -u{node:manager_user} -e "show @@backend" | awk '{print $3, $NF}' | grep true | awk '{print $1}'
    """
   Given kill mysql conns in "mysql-master1" in "master1_heartbeat_id2"
   Then execute sql in "dble-1" in "user" mode
      | user | passwd | conn   | toClose | sql                                  | expect  | db  |
      | test | 111111 | conn_1    | True    | select * from sharding_2_t1    | error totally whack | schema1 |
   Then check following text exist "Y" in file "/opt/dble/logs/dble.log" after line "log_linenu" in host "dble-1"
