...
Feature: check dbDistrict and dbDataCenter
  ...
  Scenario: shardingUser check dbDistrict and dbDataCenter value #1
 ...
 # case 5: set rwSplitMode=1, hostM2 dbDistrict=Shanghai, hostS1 and hostS2 dbDistrict=Beijing, hostS1 or hostS2 read
  ...
  Scenario: shardingUser bootstrap.cnf and db.xml only set dataCenter, local read is not enabled #2
    Given update file content "/opt/dble/conf/bootstrap.cnf" in "dble-1" with sed cmds
    """
    /-Ddistrict/d
    /-DdataCenter/d
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                                                                   | db      | expect  | timeout |
       | conn_1 | false   | drop table if exists sharding_4_t2                                    | schema1 | success |         |
       | conn_1 | false   | create table sharding_4_t2 (id int, name varchar(100), age int)       | schema1 | success |         |
       | conn_1 | false   | insert into sharding_4_t2 values (1,'name1',18),(2, 'name2',20)       | schema1 | success |         |
       | conn_1 | false   | select * from sharding_4_t2                                           | schema1 | length{(2)} | 2    |
    Then check general log in host "mysql-master2" has not "SELECT \* FROM sharding_4_t2"
    Then check general log in host "mysql-slave1" has not "SELECT \* FROM sharding_4_t2"
    Then check general log in host "mysql-slave2" has "SELECT \* FROM sharding_4_t2" occured "==2" times

    Given turn off general log in "mysql-master2"
    Given turn off general log in "mysql-slave1"
    Given turn off general log in "mysql-slave2"

  Scenario: shardingUser bootstrap.cnf and db.xml set district and dataCenter #3
    Given update file content "/opt/dble/conf/bootstrap.cnf" in "dble-1" with sed cmds
    """
    /-Ddistrict/d
    $a -Ddistrict=Shanghai
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                                                                         | db      | expect  | timeout |
       | conn_1 | false   | drop table if exists sharding_4_t3                                          | schema1 | success |         |
       | conn_1 | false   | create table sharding_4_t3 (id int, name varchar(100), age int)             | schema1 | success |         |
       | conn_1 | false   | insert into sharding_4_t3 values (1,'name1',18),(2, 'name2',20),(3,'name3',30),(4,'name4',19) | schema1 | success |         |
       | conn_1 | false   | select name from sharding_4_t3                                              | schema1 | has{(('name1',),('name2',),('name3',),('name4',))} | 2        |
    Then check general log in host "mysql-master2" has "SELECT name FROM sharding_4_t3" occured "==2" times
    Then check general log in host "mysql-slave1" has not "SELECT name FROM sharding_4_t3"
    Then check general log in host "mysql-slave2" has not "SELECT name FROM sharding_4_t3"
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                              | db      | expect      |
       | conn_1 | true    | select id from sharding_4_t3     | schema1 | length{(4)} |
    Then check general log in host "mysql-master2" has not "SELECT id FROM sharding_4_t3"
    Then check general log in host "mysql-slave1" has "SELECT id FROM sharding_4_t3" occured "==2" times
    Then check general log in host "mysql-slave2" has not "SELECT id FROM sharding_4_t3"
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                           | db      | expect      |
       | conn_1 | true    | select age from sharding_4_t3 | schema1 | length{(4)} |
    Then check general log in host "mysql-master2" has not "SELECT age FROM sharding_4_t3"
    Then check general log in host "mysql-slave1" has "SELECT age FROM sharding_4_t3" occured "==2" times
    Then check general log in host "mysql-slave2" has not "SELECT age FROM sharding_4_t3"
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                                          | db      | expect      |
       | conn_1 | true    | select age from sharding_4_t3 where age > 18 | schema1 | length{(3)} |
    Then check general log in host "mysql-master2" has not "SELECT age FROM sharding_4_t3 WHERE age > 18"
    Then check general log in host "mysql-slave1" has not "SELECT age FROM sharding_4_t3 WHERE age > 18"
    Then check general log in host "mysql-slave2" has "SELECT age FROM sharding_4_t3 WHERE age > 18" occured "==2" times
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                               | db      | expect                     |
       | conn_1 | true    | select id,name from sharding_4_t3 | schema1 | java.io.IOException: the dbGroup[ha_group2] doesn't contain active dbInstance. |
    Then check general log in host "mysql-master2" has not "SELECT id, name FROM sharding_4_t3"
    Then check general log in host "mysql-slave1" has not "SELECT id, name FROM sharding_4_t3"
    Then check general log in host "mysql-slave2" has not "SELECT id, name FROM sharding_4_t3"
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                              | db      | expect      |
       | conn_1 | true    | select id,age from sharding_4_t3 | schema1 | length{(4)} |
    Then check general log in host "mysql-master2" has not "SELECT id, age FROM sharding_4_t3"
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                                   | db      | expect      |
       | conn_1 | true    | select id,name,age from sharding_4_t3 | schema1 | length{(4)} |
    Then check general log in host "mysql-master2" has "SELECT id, name, age FROM sharding_4_t3" occured "==2" times
    Then check general log in host "mysql-slave1" has not "SELECT id, name, age FROM sharding_4_t3"
    Then check general log in host "mysql-slave2" has not "SELECT id, name, age FROM sharding_4_t3"
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                                                  | db      | expect      |
       | conn_1 | true    | select id,name,age from sharding_4_t3 where age > 20 | schema1 | length{(1)} |
    Then check general log in host "mysql-master2" has not "SELECT id, name, age FROM sharding_4_t3 WHERE age > 20"
    Then check general log in host "mysql-slave1" has not "SELECT id, name, age FROM sharding_4_t3 WHERE age > 20"
    Then check general log in host "mysql-slave2" has "SELECT id, name, age FROM sharding_4_t3 WHERE age > 20" occured "==2" times
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                                | db      | expect      |
       | conn_1 | true    | select name,age from sharding_4_t3 | schema1 | length{(4)} |
    Then check general log in host "mysql-master2" has "SELECT name, age FROM sharding_4_t3" occured "==2" times
    Then check general log in host "mysql-slave1" has not "SELECT name, age FROM sharding_4_t3"
    Then check general log in host "mysql-slave2" has not "SELECT name, age FROM sharding_4_t3"
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                                           | db      | expect                |
       | conn_1 | true    | select name,age from sharding_4_t3 where id=1 | schema1 | has{(('name1', 18),)} |
    Then check general log in host "mysql-master2" has "select name,age from sharding_4_t3 where id=1"
    Then check general log in host "mysql-slave1" has not "select name,age from sharding_4_t3 where id=1"
    Then check general log in host "mysql-slave2" has not "select name,age from sharding_4_t3 where id=1"
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                                     | db  | expect                |
       | conn_1 | true    | select name,age from sharding_4_t3 where id=3 | schema1 | has{(('name3', 30),)} |
    Then check general log in host "mysql-master2" has not "SELECT name, age FROM sharding_4_t3 WHERE id = 3"

    # case 12: hint case
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                                                                              | db      | expect      |
       | conn_1 | false   | /* master */select name,age from sharding_4_t3 where age!=20                     | schema1 | length{(3)} |
       | conn_1 | false   | /* uproxy_dest:172.100.9.6:3306 */select name,age from sharding_4_t3 where id!=3 | schema1 | length{(3)} |
       | conn_1 | false   | /*!dble:db_type=master*/ select name,age from sharding_4_t3 where age=30         | schema1 | has{(('name3', 30),)} |
       | conn_1 | false   | /*!dble:shardingNode=dn2*/ select name,age from sharding_4_t3 where age=18       | schema1 | has{(('name1', 18),)} |
       | conn_1 | false   | /*!dble:db_instance_url=172.100.9.6:3306*/ select count(0) from sharding_4_t3   | schema1 | current hint type is not supported |
       | conn_1 | true    | drop table if exists sharding_4_t3                                              | schema1 | success               |
    Then check general log in host "mysql-master2" has not "SELECT name, age FROM sharding_4_t3 WHERE age != 20"
    Then check general log in host "mysql-master2" has not "SELECT name, age FROM sharding_4_t3 WHERE id != 3"
    Then check general log in host "mysql-master2" has "SELECT name, age FROM sharding_4_t3 WHERE age = 30" occured "==2" times
    Then check general log in host "mysql-master2" has not "select name,age from sharding_4_t3 where age=18"
    Then check general log in host "mysql-master2" has not "select count(0) from sharding_4_t3"

    Given turn off general log in "mysql-master2"
    Given turn off general log in "mysql-slave1"
    Given turn off general log in "mysql-slave2"

  Scenario: shardingUser check rwStickyTime and local read #4
    Given update file content "/opt/dble/conf/bootstrap.cnf" in "dble-1" with sed cmds
    """
    /-Ddistrict/d
    $a -Ddistrict=Shanghai
    ...
    Given add xml segment to node with attribute "{'tag':'schema','kv_map':{'name':'schema1'}}" in "sharding.xml"
    """
        <shardingTable name="sharding_4_t4" shardingNode="dn1,dn2,dn3,dn4" function="hash-four" shardingColumn="id"/>
    """
    Then restart dble in "dble-1" success
    Given turn on general log in "mysql-master2"
    Given turn on general log in "mysql-slave1"
    ...
    Given execute sql in "dble-1" in "user" mode
       | conn   | toClose | sql                                                                         | db      | expect  | timeout |
       | conn_1 | false   | drop table if exists sharding_4_t4                                          | schema1 | success |         |
       | conn_1 | false   | create table sharding_4_t4 (id int, name varchar(100), age int)             | schema1 | success |         |
       | conn_1 | false   | insert into sharding_4_t4 values (1,'name1',18),(2, 'name2',20),(3,'name3',30),(4,'name4',19) | schema1 | success |         |
       | conn_1 | false   | select name from sharding_4_t4                                              | schema1 | has{(('name1',),('name2',),('name3',),('name4',))} | 2      |
       | conn_1 | true    | drop table if exists sharding_4_t4                                          | schema1 | success |         |
    Then check general log in host "mysql-master2" has not "SELECT name FROM sharding_4_t4"
    Then check general log in host "mysql-slave1" has "SELECT name FROM sharding_4_t4" occured "==2" times
    Then check general log in host "mysql-slave2" has not "SELECT name FROM sharding_4_t4"
    Given turn off general log in "mysql-master2"
    Given turn off general log in "mysql-slave1"
    Given turn off general log in "mysql-slave2"