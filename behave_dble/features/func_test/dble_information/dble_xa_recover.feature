...
Feature: check dble_xa_recover and exception xa transactions
  ...
  """
    ...
    When prepare execute 'XA COMMIT 'Dble_Server.1.1.db1.schema1.sharding_4_t1'' in dbInstance[[]name=hostM2,disabled=false,maxCon=1000,minCon=10[]] , check it's isAlive is false
    """
    ...
    Then check following text exist "Y" in file "/opt/dble/logs/dble.log" in host "dble-1"
    """
    XA COMMIT 'Dble_Server.1.1.db1.schema1.sharding_4_t1'
