...
Feature: sql_statistic_by_frontend_by_backend_by_entry_by_user
  ...
  Scenario: simple sql test #1
   ...
   Given add xml segment to node with attribute "{'tag':'root'}" in "db.xml"
    ...
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql                                                                        | expect      | db               | timeout |
      | conn_0 | False   | select * from sql_statistic_by_frontend_by_backend_by_entry_by_user | length{(4)}   | dble_information | 5       |
#      | conn_0 | False   | select * from sql_statistic_by_table_by_user_by_entry               | length{(1)}   | dble_information | 5       |
#      | conn_0 | False   | select * from sql_statistic_by_associate_tables_by_entry_by_user    | length{(0)}   | dble_information | 5       |
