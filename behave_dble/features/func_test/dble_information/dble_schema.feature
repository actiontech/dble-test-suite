...
Feature:  dble_schema test
  ...
  #case desc dble_schema
    ...
    Then execute sql in "dble-1" in "admin" mode
      ...
      | conn_0 | False   | desc dble_schema             | length{(5)}   | dble_information |
    ...
    Then check resultset "dble_schema_2" has lines with following column values
      | name-0  | sharding_node-1 | function-2 | sql_max_limit-3 | logical_create_and_drop-4 |
  ...
  #case supported select limit/order by
    Then execute sql in "dble-1" in "admin" mode
      | conn   | toClose | sql                                                                                                             | expect                                                                                                                                                                | db               |
      | conn_0 | False   | select * from dble_schema limit 1                                                                               | has{(('schema1', 'dn5', '-', 100,'true'),)}                                                                                                                                       | dble_information |
      | conn_0 | False   | select * from dble_schema order by sql_max_limit                                                                | has{(('schema3', None, '-', -1,'true'), ('schema1', 'dn5', '-', 100,'true'), ('schema2', 'dn6', '-', 1000,'true'))}                                                                                       | dble_information |
      | conn_0 | False   | select * from dble_schema order by sql_max_limit desc                                                           | has{(('schema2', 'dn6', '-', 1000,'true'), ('schema1', 'dn5', '-', 100,'true'), ('schema3', None, '-', -1,'true'))}                                                                                       | dble_information |
  ...
  #case supported select where like
      | conn_0 | False   | select * from dble_schema where name ='schema3'                                                                 | has{(('schema3', None, '-', -1,'true'),)}                                                                                                                                            | dble_information |
      | conn_0 | False   | select * from dble_schema where abs(sql_max_limit)=1                                                            | has{(('schema3', None, '-', -1,'true'),)}                                                                                                                                            | dble_information |
      | conn_0 | False   | select name from dble_schema                                                                                    | has{(('schema1',), ('schema2',), ('schema3',))}                                                                                                                       | dble_information |
  #case supported select join
      | conn_0 | False   | select * from dble_schema a left join dble_sharding_node b on a.sharding_node =b.name order by a.sql_max_limit  | has{(('schema3', None, '-', -1,'true', None, None, None, None), ('schema1', 'dn5', '-', 100,'true','dn5','ha_group1','db3','false'),('schema2','dn6', '-', 1000,'true','dn6','ha_group2','db3','false'))} | dble_information |
      | conn_0 | False   | select * from dble_schema a right join dble_sharding_node b on a.sharding_node =b.name where a.name='schema1'   | has{(('schema1','dn5', '-', 100,'true','dn5','ha_group1','db3','false'),)}                                                                                                          | dble_information |
      | conn_0 | False   | select * from dble_schema a inner join dble_sharding_node b on a.sharding_node =b.name order by a.sql_max_limit | has{(('schema1','dn5', '-', 100,'true','dn5','ha_group1','db3','false'),('schema2','dn6', '-', 1000,'true','dn6','ha_group2','db3','false'))}                                                   | dble_information |
  #case supported select where [sub-query]
      | conn_0 | False   | select * from dble_schema where sharding_node in (select name from dble_sharding_node )                         | has{(('schema1', 'dn5', '-', 100,'true'), ('schema2', 'dn6', '-', 1000,'true'),)}                                                                                                             | dble_information |
      ...
      | conn_0 | False   | select * from dble_schema where sharding_node < any (select name from dble_sharding_node)                       | has{(('schema1', 'dn5', '-', 100,'true'),) }                                                                                                                                        | dble_information |
      | conn_0 | False   | select * from dble_schema where sharding_node = any (select name from dble_sharding_node )                      | has{(('schema1', 'dn5', '-', 100,'true'), ('schema2', 'dn6', '-', 1000,'true'),)}                                                                                                             | dble_information |
      | conn_0 | False   | select * from dble_schema where sharding_node in (select name from dble_sharding_node where db_group ='ha_group1')      | has{(('schema1', 'dn5', '-', 100,'true'),)}           | dble_information |
