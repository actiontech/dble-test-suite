...
#case https://github.com/actiontech/dble/issues/1913
   ...
   Scenario: 'select ...having' then there's the 'or' condition from github:2073/2158/2199 #3
    Then execute sql in "dble-1" in "user" mode
      ...
      | conn_0 | False   | select c from sharding_4_t1 group by c having id<=1 or c = 'bb'                            | length{(2)} | schema1 |
      | conn_0 | False   | explain select c from sharding_4_t1 group by c having id<=1 or c = 'bb'                    | success     | schema1 |
