...
Feature: verify issue 92 #Enter feature name here
  ...
  @restore_mysql_config @use.with_mysql_version=5.7
  Scenario: check support utf8mb4: case from issue DBLE0REQ-582 #2
   ...
   Given add xml segment to node with attribute "{'tag':'root'}" in "sharding.xml"
    ...
    Then execute sql in "dble-1" in "user" mode
      ...
      | conn_0 | False    | drop table if exists shard15        | success      | schema1 | utf8mb4 |
      ...
      | conn_0 | False    | CREATE TABLE `shard15` ( `CST_AT_IDNO` int(16) NOT NULL COMMENT '客识',   `OG_NO` varchar(32) NOT NULL COMMENT '供23位',   `tg_id` bigint(40) DEFAULT NULL,   `T_O123` varchar(32) NOT NULL COMMENT '台23号',   `TG_NAME` varchar(256) DEFAULT NULL COMMENT '台区55称',`AGENT` varchar(32) NOT NULL COMMENT '人号',  `AGENT_NAME` varchar(64) NOT NULL COMMENT '人名',   `V4D_FL33` varchar(8) DEFAULT NULL COMMENT '效)',  `O09R` varchar(32) DEFAULT NULL COMMENT '人',   `OP_ATE` date DEFAULT NULL COMMENT '操作时间',PRIMARY KEY (`CST_AT_IDNO`) USING BTREE,   KEY `IDX_shard15_ON` (`OG_NO`) USING BTREE ) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='客379'  | success | schema1 | utf8mb4 |

      | conn_1 | true    | select IFNULL(SUM(t1.DA12_VALE), 0) dataValue, IFNULL(SUM(t1.DA12_V123_S43), 0) dataValueLy, s.mon everyDate, 'JY440' idxNo, '41401' orgC66E, 'IND_08_CJCGLJK' themeNo, '01' statCalibre,'03' busiC66E, '08' majorNo from(select t.DA12_VALE, t.DA12_V123_S43, t.ben_tim12 from shard12 t where t.OG_NO = '41401' and t.ben_tim12 in (select DATE_FORMAT(y.ymr, '%Y%m%d') ymr from(select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 1 MONTH))< CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 1 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 2 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 2 MONTH)),CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 3 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 3 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1,INTERVAL - 4 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 4 MONTH)), CURDATE()) ymr union select  if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 5 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1,INTERVAL - 5 MONTH)), CURDATE()) ymr union select  if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 6 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 6 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 7 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 7 MONTH)), CURDATE()) ymr union select  if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL- 8 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 8 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 9 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1,INTERVAL - 9 MONTH)), CURDATE()) ymr union select  if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 10 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 10 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 11 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 11 MONTH)), CURDATE()) ymr union SELECT CURDATE() ymr) y where y.ymr > DATE_SUB(CURDATE(), INTERVAL DAYOFYEAR(NOW()) - 1 DAY)) and t.TE_N = 'IND_08_CJCGLJK' and t.IX123_NO = 'JY440' and t.BI_CE = '03' and t.MO_N = '08' and t.SAT_CRE = '01') t1 RIGHT outer JOIN(select s1. `NAME`  mon, s1. `C66E`  monC66E from shard13 s1 where s1.DT_TYPEID = 'month_C66E') s on SUBSTRING(t1.ben_tim12, 5, 2) = s.monC66E GROUP BY s.monC66E ORDER BY s.monC66E asc                                                                                                                                       | success | schema1 | utf8mb4 |
      | conn_1 | true    | select IFNULL(SUM(t1.DA12_VALE), 0) dataValue, IFNULL(SUM(t1.DA12_V123_S43), 0) dataValueLy, s.mon everyDate, 'JY440' idxNo, '41401' orgC66E, 'IND_08_CJCGLJK' themeNo, '01' statCalibre, '03' busiC66E, '08' majorNo from(select t.DA12_VALE, t.DA12_V123_S43, t.ben_tim12 from shard12 t where t.OG_NO = '41401'     and t.ben_tim12 in (select DATE_FORMAT(y.ymr, '%Y%m%d') ymr from(select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 1 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 1 MONTH)), CURDATE()) ymr union select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 2 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 2 MONTH)), CURDATE()) ymr union select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 3 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 3 MONTH)), CURDATE()) ymr union select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 4 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 4 MONTH)), CURDATE()) ymr union select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 5 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 5 MONTH)), CURDATE()) ymr union select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 6 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 6 MONTH)), CURDATE()) ymr union select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 7 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 7 MONTH)), CURDATE()) ymr union select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 8 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 8 MONTH)), CURDATE()) ymr union select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 9 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 9 MONTH)), CURDATE()) ymr union select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 10 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 10 MONTH)), CURDATE()) ymr union select         if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 11 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 11 MONTH)), CURDATE()) ymr union SELECT CURDATE() ymr) y where y.ymr > DATE_SUB(CURDATE(), INTERVAL DAYOFYEAR(NOW()) - 1 DAY)) and t.TE_N = 'IND_08_CJCGLJK'     and t.IX123_NO = 'JY440'     and t.BI_CE = '03'     and t.MO_N = '08'     and t.SAT_CRE = '01') t1 RIGHT outer JOIN(select s1.     `NAME`     mon, s1.     `C66E`     monC66E from shard13 s1 where s1.DT_TYPEID = 'month_C66E') s on SUBSTRING(t1.ben_tim12, 5, 2) = s.monC66E GROUP BY s.monC66E ORDER BY s.monC66E asc                                       | success | schema1 | utf8mb4 |
      | conn_1 | true    | select IFNULL(SUM(t1.DA12_VALE), 0) dataValue, IFNULL(SUM(t1.DA12_V123_S43), 0) dataValueLy, s.mon everyDate, ':idxNo' idxNo, ':orgC66E' orgC66E, ':themeNo' themeNo, ':statCalibre' statCalibre, ':busiC66E' busiC66E, ':majorNo' majorNo, ':statTime' statTime, ':agent' agent from(select t.DA12_VALE, t.DA12_V123_S43, t.ben_tim12 from shard12 t, shard15 tg where t.OG_NO = ':orgC66E' and t.OG_NO = tg.OG_NO and tg.T_O123 = t.T_O123 and tg.V4D_FL33 = '1' and t.ben_tim12 in (select DATE_FORMAT(y.ymr, '%Y%m%d') ymr from(select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 1 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 1 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 2 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 2 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 3 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 3 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 4 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 4 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 5 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 5 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 6 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 6 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 7 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 7 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 8 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 8 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 9 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 9 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 10 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 10 MONTH)), CURDATE()) ymr union select if (LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 11 MONTH)) < CURDATE(), LAST_DAY(DATE_ADD(CURDATE() - DAY(CURDATE()) + 1, INTERVAL - 11 MONTH)), CURDATE()) ymr union SELECT CURDATE() ymr) y where y.ymr > DATE_SUB(CURDATE(), INTERVAL DAYOFYEAR(NOW()) - 1 DAY)) and t.TE_N = ':themeNo' and t.IX123_NO = ':idxNo' and t.BI_CE = ':busiC66E' and t.MO_N = ':majorNo' and t.SAT_CRE = ':statCalibre' and tg.AGENT = ':agent') t1 RIGHT OUTER JOIN(select s1. `NAME` mon, s1. `C66E` monC66E from shard13 s1 where s1.DT_TYPEID = 'month_C66E') s on SUBSTRING(t1.ben_tim12, 5, 2) = s.monC66E GROUP BY s.monC66E ORDER BY s.monC66E asc | success | schema1 | utf8mb4 |
      | conn_1 | true    | SELECT d.NAME NAME,d.C66E C66E,IFNULL(c.采败, 0 ) 采败, IFNULL(c.采异, 0 ) 采异,IFNULL(c.采台区, 0 ) 采台区,IFNULL(c.倍误, 0 ) 倍误,IFNULL(c.三衡, 0 ) 三衡,IFNULL(c.爱可生, 0 ) 爱可生,IFNULL(c.喝喝喝, 0 ) 喝喝喝,IFNULL(c.什么, 0 ) 什么,IFNULL(c.含区, 0 ) 含区,IFNULL(c.存, 0 ) 存 FROM ( SELECT O.orgno, IFNULL( max( CASE WHEN O.VALUE = '01' THEN O.NUM END ), 0 ) 采败, IFNULL( max( CASE WHEN O.VALUE = '02' THEN O.NUM END ), 0 ) 采异, IFNULL( max( CASE WHEN O.VALUE = '03' THEN O.NUM END ), 0 ) 采台区, IFNULL( max( CASE WHEN O.VALUE = '04' THEN O.NUM END ), 0 ) 倍误, IFNULL( max( CASE WHEN O.VALUE = '05' THEN O.NUM END ), 0 ) 三衡, IFNULL( max( CASE WHEN O.VALUE = '06' THEN O.NUM END ), 0 ) 爱可生, IFNULL( max( CASE WHEN O.VALUE = '07' THEN O.NUM END ), 0 ) 喝喝喝, IFNULL( max( CASE WHEN O.VALUE = '08' THEN O.NUM END ), 0 ) 什么, IFNULL( max( CASE WHEN O.VALUE = '09' THEN O.NUM END ), 0 ) 含区, IFNULL( max( CASE WHEN O.VALUE = '10' THEN O.NUM END ), 0 ) 存 FROM ( SELECT A.OG_NO orgno, A.DI78M234 VALUE , ( CASE WHEN DI78M234 = '01' THEN '采败' WHEN DI78M234 = '02' THEN '采异' WHEN DI78M234 = '03' THEN '采台区' WHEN DI78M234 = '04' THEN '倍误' WHEN DI78M234 = '05' THEN '三衡' WHEN DI78M234 = '06' THEN ' 爱可生' WHEN DI78M234 = '07' THEN '喝喝喝' WHEN DI78M234 = '08' THEN '什么' WHEN DI78M234 = '09' THEN '含区' WHEN DI78M234 = '10' THEN '存' END ) NAME, round( sum( A.DA12_VALE )) num FROM shard12 A WHERE A.TE_N = 'IND_08_GSFSYYQKJK' AND A.ben_tim12 = ':statTime' AND A.OG_NO LIKE ':orgC66E%' AND LENGTH( A.OG_NO )= 7 AND A.DI23M123 = '01' GROUP BY A.OG_NO, A.DI23M123, A.DI78M234 ORDER BY DI78M234 ASC ) O GROUP BY O.orgno ) c RIGHT JOIN ( SELECT * FROM shard13 WHERE C66E LIKE ':orgC66E%' AND LENGTH( C66E ) = 7 AND NAME NOT LIKE '%爱可生社区%' AND NAME NOT LIKE '%客户中心%' ) d ON c.orgno = d.C66E ORDER BY ( d.C66E = '4140601' ) DESC, d.C66E                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | success | schema1 | utf8mb4 |
      | conn_1 | true    | SELECT ttt.EACHDAY statTime, CONCAT(if(SUBSTRING( eachDay, 7, 2 ) > 9, SUBSTRING( eachDay, 7, 2 ),SUBSTRING( eachDay, 8, 1 )),'日') xValue, IFNULL( ddd.DA12_VALE, 0 ) dataValue, IFNULL( ddd.DA12_V123_S43, 0 ) dataValueLy, ':orgNo' orgN, 'lll411010334472' idxNo, ddd.SAT_CRE statCalibre FROM (SELECT REPLACE(A.DATE,'-','') eachDay FROM ( SELECT CURDATE() - INTERVAL(A.A+(10*B.A)+(100*C.A)-1) DAY AS DATE FROM (SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9) AS A CROSS JOIN(SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 ) AS B CROSS JOIN(SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 ) AS C ) A WHERE A.DATE BETWEEN DATE_SUB(DATE_FORMAT(':statTime','%y%m%d'),INTERVAL 12 day) AND DATE_FORMAT(':statTime','%y%m%d') ORDER BY eachDay ) ttt LEFT JOIN ( SELECT cidm.ben_tim12, round(sum(case when DI23M123 = '01' then DA12_VALE else 0 end)/sum(DA12_VALE) * 100,2) DA12_VALE, round(sum(case when DI23M123 = '01' then DA12_V123_S43 else 0 end)/sum(DA12_V123_S43) * 100,2) DA12_V123_S43, ':orgNo' orgNo, cidm.SAT_CRE FROM shard12 cidm WHERE OG_NO = ':orgNo' and SAT_CRE = ':statCalibre' AND ben_tim12 in (SELECT REPLACE(A.DATE,'-','') eachDay FROM ( SELECT CURDATE() - INTERVAL(A.A+(10*B.A)+(100*C.A)-1) DAY AS DATE FROM (SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9) AS A CROSS JOIN(SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 ) AS B CROSS JOIN(SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 ) AS C ) A WHERE A.DATE BETWEEN DATE_SUB(DATE_FORMAT(':statTime','%y%m%d'),INTERVAL 12 day) AND DATE_FORMAT(':statTime','%y%m%d') ORDER BY eachDay) AND TE_N = 'IND_08_GSFSTQJK' AND IX123_NO = 'lll411010334344' GROUP BY IX123_NO, ben_tim12 ORDER BY ben_tim12 ) ddd ON ttt.EACHDAY = ddd.ben_tim12 ORDER BY ttt.EACHDAY                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | success | schema1 | utf8mb4 |
      | conn_1 | true    | SELECT ttt.EACHDAY statTime, CONCAT(if(SUBSTRING( eachDay, 7, 2 ) > 9, SUBSTRING( eachDay, 7, 2 ),SUBSTRING( eachDay, 8, 1 )),'日') xValue, IFNULL( ddd.DA12_VALE, 0 ) dataValue, IFNULL( ddd.DA12_V123_S43, 0 ) dataValueLy, ':orgNo' orgNo, ddd.SAT_CRE statCalibre FROM (SELECT REPLACE(A.DATE,'-','') eachDay FROM ( SELECT CURDATE() - INTERVAL(A.A+(10*B.A)+(100*C.A)-1) DAY AS DATE FROM (SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9) AS A CROSS JOIN(SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 ) AS B CROSS JOIN(SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 ) AS C ) A WHERE A.DATE BETWEEN DATE_SUB(DATE_FORMAT(':statTime','%y%m%d'),INTERVAL 12 day) AND DATE_FORMAT(':statTime','%y%m%d') ORDER BY eachDay ) ttt LEFT JOIN ( SELECT cidm.ben_tim12, round(sum(case when IX123_NO = 'JY000' then DA12_VALE else 0 end)/sum(case when IX123_NO = 'lll411010334628' then DA12_VALE else 0 end) * 100,2) DA12_VALE, round(sum(case when IX123_NO = 'JY000' then DA12_V123_S43 else 0 end)/sum(case when IX123_NO = 'lll411010334628' then DA12_V123_S43 else 0 end) * 100,2) DA12_V123_S43, ':orgNo' orgNo, cidm.SAT_CRE FROM shard12 cidm WHERE OG_NO = ':orgNo' and SAT_CRE = ':statCalibre' AND ben_tim12 in (SELECT REPLACE(A.DATE,'-','') eachDay FROM ( SELECT CURDATE() - INTERVAL(A.A+(10*B.A)+(100*C.A)-1) DAY AS DATE FROM (SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9) AS A CROSS JOIN(SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 ) AS B CROSS JOIN(SELECT 0 AS A UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 ) AS C ) A WHERE A.DATE BETWEEN DATE_SUB(DATE_FORMAT(':statTime','%y%m%d'),INTERVAL 12 day) AND DATE_FORMAT(':statTime','%y%m%d') ORDER BY eachDay) AND TE_N = 'IND_08_XSHGL' GROUP BY ben_tim12 ORDER BY ben_tim12 ) ddd ON ttt.EACHDAY = ddd.ben_tim12 ORDER BY ttt.EACHDAY                                                                                                                                                                                                                                                                                                                                                                                                                                                     | success | schema1 | utf8mb4 |
...
#case the result support utf8 ATK-1398
    ...
    Then check resultset "6" has lines with following column values
      ...
      | 2023年    | None       | None                 | 0           | :statTime       | 41101   |
