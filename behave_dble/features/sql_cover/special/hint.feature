...
Feature: verify hint sql
  ...
  Scenario: sql from GUI CLient test    author:maofei #8
    ...
    Scenario: hint for specail sql syntax: execute multi-statement in create procedure #9
    Given add xml segment to node with attribute "{'tag':'schema','kv_map':{'name':'schema1'}}" in "schema.xml"
      """
        <table name="test_shard" dataNode="dn1,dn3" rule="hash-two" />
     """
    ...
    Then execute sql in "dble-1" in "user" mode
        ...
        | test | 111111 | conn_0 | False    |  delimiter $$                   |  | schema1 |
        ...
        | test | 111111 | conn_0 | False    | delimiter ;          |  | schema1 |
        | test | 111111 | conn_0 | True     | /*!dble:sql=select * from test_shard where id =1*/call delete_matches(1)          | success | schema1 |
