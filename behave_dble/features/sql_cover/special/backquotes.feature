...
Feature: verify table name or schema name enclosed by backquotes can work fine
  ...
  Scenario: explain right when table name enclosed by backquotes #1
    ...
    Then execute sql in "dble-1" in "user" mode
      ...
      | test | 111111 | conn_0 | False    | create table sharding_4_t1(id int,name char(20))                    | success  | schema1 |
      | test | 111111 | conn_0 | True    | insert into sharding_4_t1 values(1,'aaa'),(2,'bbb'),(3,'ccc') | success  | schema1 |
  ...
  Scenario: execute with hint when explain table name enclosed by backquotes,explain right when column alias enclosed by backquotes #2
    ...
    Then execute sql in "dble-1" in "user" mode
      | user | passwd | conn   | toClose  | sql                                                                       | expect   | db      |
      | test | 111111 | conn_0 | False    |   drop table if exists sharding_enum_string_t1 | success  | schema1 |
      | test | 111111 | conn_0 | False    |   create table sharding_enum_string_t1(id_0 int,id char(3)) | success  | schema1 |
      | test | 111111 | conn_0 | False    |   insert into sharding_enum_string_t1 values(1,'aaa'),(2,'bbb'),(3,'ccc') | success  | schema1 |
      | test | 111111 | conn_0 | False    |   select * from sharding_enum_string_t1 | length{(3)}  | schema1 |
      | test | 111111 | conn_0 | False    |  /*!dble:datanode=dn2*/ select * from `sharding_enum_string_t1` where id='bbb'   | length{(1)}  | schema1 |
      ...
      | test | 111111 | conn_0 | False    |   selviect `a`.`id_0` as `sid` from `sharding_enum_string_t1` as `a` where id in ('aaa', 'bbb') having `sid` > 1 | hasStr{(2L,)}  | schema1 |
