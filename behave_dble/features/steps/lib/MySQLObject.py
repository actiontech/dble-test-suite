...
class MySQLObject(object):
    ...
    def kill_all_conns(self, exclude_conn_ids=None):
        ...
        for row in res:
            ...
            not_event_scheduler = row[1] != "event_scheduler"#for excluding event_scheduler, mysql8.0 event_scheduler is on
            if not_event_scheduler and not_show_processlist and (exclude_conn_ids is None or str(conn_id) not in exclude_conn_ids):
    ...
    def kill_conns(self, conn_ids):
        ...
        for row in res:
            ...
            not_show_processlist = row[7] != "show processlist"#for excluding show processlist conn itself
            not_event_scheduler = row[1] != "event_scheduler"  # for excluding event_scheduler, mysql8.0 event_scheduler is on
            if not_event_scheduler and not_show_processlist and str(conn_id) in conn_ids:
                res, err = conn.execute("kill {}".format(conn_id))
                assert err is None, "kill conn '{}' failed for {}".format(conn_id,err[1])
    ...
    def kill_redundant_conns(self, conn_ids_origin, kill_num):
        ...
        for row in res:
            ...
            not_show_processlist = row[7] != "show processlist"#for excluding show processlist conn itself
            not_event_scheduler = row[1] != "event_scheduler"#for excluding event_scheduler, mysql8.0 event_scheduler is on
            if not_event_scheduler and not_show_processlist and conn_id in conn_ids:
                res, err = conn.execute("kill {}".format(conn_id))
                assert err is None, "kill conn '{}' failed for {}".format(conn_id,err[1])
