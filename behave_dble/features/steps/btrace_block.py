...
def run_dble_query(sshClient, context):
    ...
    for row in context.table:
        ...
        cmd = u"nohup {0}/bin/mysql -u{1} -p{2} -P{3} -c -D{4} -e'{5}' >/tmp/dble_query.log 2>&1 &".format(context.cfg_mysql['install_path'], user, passwd,context.cfg_dble['manager_port'], db, sql)
        rc, sto, ste = sshClient.exec_command(cmd)
        assert len(ste)==0, "impossible err occur"
