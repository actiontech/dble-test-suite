Feature:test user's privileges under different combination
  ...
  Scenario: add readonly client user, user can only read to table in schema's default node and to global table #2
    ...
    Then execute sql in "dble-1" in "user" mode
        ...
        | test_user | test_password | conn_1 | False   | create table test_table(id int, data varchar(10))  | User READ ONLY |mytest |
        | test      | 111111        | conn_0 | False   | create table test_table(id int, data varchar(10))  | success        |mytest |
        | test_user | test_password | conn_1 | False   | drop table test_table                              | User READ ONLY | mytest |
        | test_user | test_password | conn_1 | False   | alter table test_table add column data1 varchar(10)| User READ ONLY | mytest |
        | test_user | test_password | conn_1 | False   | insert into test_table values (1, 'aaa')           | User READ ONLY | mytest |
        | test_user | test_password | conn_1 | False   | update test_table set data = 'bbb' where id = 1    | User READ ONLY | mytest |
        | test_user | test_password | conn_1 | False   | delete from test_table                             | User READ ONLY | mytest |
