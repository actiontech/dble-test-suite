Feature:#test reference manager cmd  and main function for slow query log
   ...
   Scenario:# 2.test main function of slow query log
     ...
     """
      Given Restart dble in "dble-1"
      Then check following " " exist in "/opt/dble/" in "dble-1"
      """
      dir
      slowQuery
      query.log
      ...
      Then check following "not" exist in "/opt/dble/slowQuery/query.log" in "dble-1"
      """
    ...
    Then execute sql in "dble-1" in "admin" mode
      ...
      Then check following " " exist in "/opt/dble/slowQuery/query.log" in "dble-1"
      """
      drop table if exists a_test
      create table a_test(id int)
      alter table a_test add name char(20)
