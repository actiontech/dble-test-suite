...
void case_mysql_options4(MYSQL* conn){
	...
	if(IS_DEBUG){
        mysql_real_connect(&mysql, Host_Single_MySQL, TEST_USER, TEST_USER_PASSWD, NULL, 0,NULL, CLIENT_DEPRECATE_EOF|CLIENT_MULTI_STATEMENTS);
	}else{
        mysql_real_connect(&mysql, Host_Test, TEST_USER, TEST_USER_PASSWD, NULL, 0,NULL, CLIENT_DEPRECATE_EOF|CLIENT_MULTI_STATEMENTS);
	}
