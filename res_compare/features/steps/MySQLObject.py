...
from steps.utils import update_file_with_sed,get_mysql_cnf_path
...
class MySQLObject(object):
    ...
    def connect_test(self):
        ...
        while conn is None:
            try:
                conn = Connector(host=self._mysql_meta.ip, user=self._mysql_meta.mysql_user,
    ...
    def turn_on_general_log(self, to_clean_old_file=True):
        ...
        conn = Connector(host=self._mysql_meta.ip, user=self._mysql_meta.mysql_user, passwd=self._mysql_meta.mysql_password, db='',
                             port=self._mysql_meta.mysql_port, autocommit=True)
